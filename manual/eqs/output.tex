...
in Section \para\ref{sec:ww3shel}. That input file also provides a list of flags  
...
a second group line is expected containing parameter flags. Example of F/T flag 
use is given in  \para\ref{sec:ww3shel}.
All parameters that are a function of frequency (e.g.  \textbf{EF} or \textbf{USF}) require the setting 
of specific namelist parameters, in the OUTS namelist. This is to reduce the memory use
if these parameters are not needed. 

For Namelists, the first line is set to N, and the next line contains parameter
symbols as per table in Section \para\ref{sec:ww3shel}.  An example is given in  
...
in the first two columns in the field output parameter itable of 
...
\item \textbf{T0M1} Mean wave period (s) [see Eq.~(\ref{eq:zbar})]
      \begin{equation} T_{m-1,0} = 2\pi \overline{\sigma^{-1}}
...
\item \textbf{TH2M} Mean direction from $a_2$ and $b_2$ (degr.) (NOT PLUGGED YET)
\item \textbf{STH2M} Spread from $a_2$ and $b_2$ (degr.)  (NOT PLUGGED YET)
...
\item Directional spread of partition of the spectrum
      Cf. Eq.~(\ref{eq:sig_th}).
\item \textbf{PWS} Wind sea fraction of partition of the spectrum (see below).
...
\item \textbf{TWA} Net wave-supported stress (wind to wave momentum flux) 
...
\item \textbf{WCM} Mean breaking wave height (m) (NOT PLUGGED YET)
...
\item \textbf{USS} Stokes drift at the sea surface (m/s)
     \begin{equation} (U_{ssx},U_{ssy}) =  g \int \!\!\!\! \int  \sigma \cosh 2kd \frac{(k \cos(\theta),k \sin(\theta))}
...
\item \textbf{USF} Frequency spectrum of Stokes drift at the sea surface (m/s/Hz)
     \begin{equation} (U_{ssx}(f),U_{ssy}(f)) =  g \int \!\!\!\! \int  \sigma \cosh 2kd \frac{(k \cos(\theta),k \sin(\theta))}
