...
\opthead{ST6}{AUSWEX, Lake George}{A. Babanin, I. Young, M. Donelan, E. Rogers, S. Zieger}
...
$u_\star=2.026$m~s $^{-1}$. {\it Important!} In ST6, bulk adjustment to any
...
\paragraph{Negative Input.} Apart from the positive input, ST6 also has a 
...
\citep{art:YB06,art:Bea10} (the last feature was not implemented in ST6).
...
\caption{Summary of calibration parameters for ST6 source terms.
...
academic tests. Calibration coefficients used in ST6 and listed in
...
for wind speed is customizable in the wind input parameterization.
...
parameterization {\code FLX4} through the {\F FLX4} namelist parameter {\code
 CDFAC=1.0E-4}.  This has a similar effect to tuning variable
...
When the discrete interaction approximation (DIA) is used to compute the four-wave
