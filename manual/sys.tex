...
\noindent
There are presently four propagation schemes available, as well as a 'slot'
for a user supplied propagation routine. The propagation schemes are packaged
...
\noindent
Input and dissipation module \citep[\wam-4 and variants,][]{art:ACC08} \hfill {\file w3src3md.ftn}
...
\fit{inptab}{Same as in w3src2md.ftn.}
...
\begin{flist}
\fit{constants.ftn}{Physical and mathematical constants}
...
Fig.~\ref{fig:grids_1}. The grid can either be a Cartesian `$(x,y)$' grid or a
spherical grid. In a spherical grid, the longitudes are denoted throughout the
...
boundary points. The minimum grid size therefore is {\F nx=3, ny=3}. Input
...
\noindent
considering that only the first few bits of {\F mapsta} contain data. the
...
\vit{ddelta2}{r }{Weight of $k^2$ part in WAM4 dissipation.
                                                   \hfill ({\F !/st3})}
\vit{ssdslf}{r }{WAM4 switch  in non-saturated part.
                                                   \hfill ({\F !/st3})}
\vit{ssdshf}{r }{WAM4 switch in saturated part of spectrum.
                                                   \hfill ({\F !/st3})}
\vit{ssdsc2}{r }{Constant for SAT-part of dissipation.
                                                   \hfill ({\F !/st3})}
