...
!/PDLIB      USE W3PARALL, ONLY: SYNCHRONIZE_IPGL_ETC_ARRAY, ISEA_TO_JSEA
!/PDLIB      use yowNodepool, only: npa
!/PDLIB      use yowRankModule, only : rank
     USE W3GDATMD, ONLY: GTYPE, UNGTYPE
!/PDLIB      USE PDLIB_W3PROFSMD, ONLY : PDLIB_MAPSTA_INIT, VA_SETUP_IOBPD
!/PDLIB      USE PDLIB_W3PROFSMD, ONLY : BLOCK_SOLVER_INIT, PDLIB_STYLE_INIT
!/PDLIB      use yowDatapool, only: istatus
!/SETUP      USE W3WAVSET, ONLY : PREPARATION_FD_SCHEME
...
!/PDLIB!/DEBUGCOH     USE PDLIB_W3PROFSMD, ONLY: ALL_VA_INTEGRAL_PRINT, TEST_MPI_STATUS
!/PDLIB!/DEBUGINIT    USE PDLIB_W3PROFSMD, ONLY: PRINT_WN_STATISTIC
...
!/DEBUGINIT      WRITE(740+IAPROC,*) 'Beginning of W3INIT'
!/DEBUGINIT      WRITE(740+IAPROC,*) '  FLGR2(10,1)=', FLGR2(10,1)
!/DEBUGINIT      WRITE(740+IAPROC,*) '  FLGR2(10,2)=', FLGR2(10,2)
!/DEBUGINIT      FLUSH(740+IAPROC)
!/TIMINGS       CALL PRINT_MY_TIME("Case 2")


!/MEMCHECK       WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_INIT SECTION 1'
...
!/DEBUGINIT       WRITE(*,*) '1: w3initmd f=', TRIM(FNMPRE(:J)//LFILE(:IFL))
...
!/DEBUGINIT       WRITE(*,*) '2: w3initmd f=', TRIM(FNMPRE(:J)//TFILE(:IFT))
          IF ( .NOT. OPENED ) OPEN                                    &
...
!
!/MEMCHECK       WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_INIT SECTION 2'
!/MEMCHECK       call getMallocInfo(mallinfos)
!/MEMCHECK       call printMallInfo(IAPROC,mallInfos)
!
...
!/MEMCHECK       WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_INIT SECTION 2a'
!/MEMCHECK       call getMallocInfo(mallinfos)
!/MEMCHECK       call printMallInfo(IAPROC,mallInfos)
!
...
!/SETUP      CALL PREPARATION_FD_SCHEME(IMOD)
!/PDLIB    ELSE
!/PDLIB!/DEBUGINIT      WRITE(*,*) 'Before PDLIB_STYLE_INIT, IMOD=', IMOD
!/PDLIB      CALL PDLIB_STYLE_INIT(IMOD)
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) 'After set up of NSEAL, NSEALM=', NSEALM
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) 'After PDLIB_STYLE_INIT'
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) 'allocated(ISEA_TO_JSEA)=', allocated(ISEA_TO_JSEA)
!/PDLIB!/DEBUGINIT     FLUSH(740+IAPROC)
!/TIMINGS       CALL PRINT_MY_TIME("After PDLIB_STYLE_INIT")

!/PDLIB!/DEBUGINIT      WRITE(*,*) 'After PDLIB_STYLE_INIT, IMOD=', IMOD
!/PDLIB      CALL SYNCHRONIZE_IPGL_ETC_ARRAY(IMOD, IsMulti)
!/PDLIB    END IF
! Update of output parameter flags based on mod_def parameters (for 3D arrays)
!/DEBUGINIT      WRITE(740+IAPROC,*) 'Before W3FLGRDUPDT'
!/DEBUGINIT      FLUSH(740+IAPROC)
      CALL W3FLGRDUPDT ( NDSO, NDSE, FLGRD, FLGR2, FLGD, FLG2 )
!!/DEBUGMPI     CALL TEST_MPI_STATUS("Case 9")
!/TIMINGS       CALL PRINT_MY_TIME("After W3FLGRDUPDT")
...
!/MEMCHECK       WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_INIT SECTION 2c'
!/MEMCHECK       call getMallocInfo(mallinfos)
!/MEMCHECK       call printMallInfo(IAPROC,mallInfos)
!
!/DEBUGINIT     WRITE(740+IAPROC,*) 'After set up of NSEAL, NSEAL=', NSEAL
!/DEBUGINIT     WRITE(740+IAPROC,*) 'After set up of NSEAL, NSEALM=', NSEALM
!/DEBUGINIT     WRITE(740+IAPROC,*) 'NSEA=', NSEA, ' NSPEC=', NSPEC
!/DEBUGINIT     FLUSH(740+IAPROC)
!!/DEBUGMPI     CALL TEST_MPI_STATUS("Case 11")

!/DIST        IF ( NSEA .LT. NAPROC ) GOTO 820
!/DIST        IF ((LPDLIB .eqv. .FALSE.).or.(GTYPE .NE. UNGTYPE)) THEN
!/DIST          IF ( NSPEC .LT. NAPROC ) GOTO 821
!/DIST        END IF
!/DEBUGINIT     WRITE(740+IAPROC,*) 'Before PDLIB related allocations'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB     IF ((IAPROC .LE. NAPROC).and.(GTYPE .eq. UNGTYPE)) THEN
!/DEBUGINIT     WRITE(740+IAPROC,*) 'After test 1'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB       IF (FSNIMP .or. FSTOTALIMP) THEN
!/DEBUGINIT     WRITE(740+IAPROC,*) 'Before BLOCK_SOLVER_INIT'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB         CALL BLOCK_SOLVER_INIT()
!/DEBUGINIT     WRITE(740+IAPROC,*) 'After BLOCK_SOLVER_INIT'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/TIMINGS       CALL PRINT_MY_TIME("After BLOCK_SOLVER_INIT")
!/PDLIB       ELSE IF (FSTOTALEXP) THEN
!/DEBUGINIT     WRITE(740+IAPROC,*) 'Before AC_tot allocation'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB         allocate(AC_tot(NSPEC, npa), stat=istat)
!/DEBUGINIT     WRITE(740+IAPROC,*) 'After AC_tot allocation'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB       ENDIF
!/PDLIB     END IF
!/MEMCHECK       WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_INIT SECTION 2d'
!/MEMCHECK       call getMallocInfo(mallinfos)
!/MEMCHECK       call printMallInfo(IAPROC,mallInfos)
...
!/DEBUGINIT     WRITE(740+IAPROC,*) 'Calling W3DIMW at W3INIT, case 1'
!/DEBUGINIT     FLUSH(740+IAPROC)
          CALL W3DIMW ( IMOD, NDSE, NDST )
        ELSE
!/DEBUGINIT     WRITE(740+IAPROC,*) 'Calling W3DIMW at W3INIT, case 2'
!/DEBUGINIT     FLUSH(740+IAPROC)
          CALL W3DIMW ( IMOD, NDSE, NDST, .FALSE. )
        END IF
!/DEBUGINIT     WRITE(740+IAPROC,*) ' 1: NSEAL=', NSEAL
!/DEBUGINIT     WRITE(740+IAPROC,*) ' maxval(UST)=', maxval(UST)
!/DEBUGINIT     FLUSH(740+IAPROC)
!/TIMINGS       CALL PRINT_MY_TIME("After W3DIMW")
      CALL W3DIMA ( IMOD, NDSE, NDST )
      CALL W3DIMI ( IMOD, NDSE, NDST , FLAGSTIDEIN )
!!/DEBUGMPI     CALL TEST_MPI_STATUS("Case 13")
!/TIMINGS       CALL PRINT_MY_TIME("After W3DIMI")

!/MEMCHECK      WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_INIT SECTION 3'
!/MEMCHECK      call getMallocInfo(mallinfos)
!/MEMCHECK      call printMallInfo(IAPROC,mallInfos)
!
...
!/TIMINGS       CALL PRINT_MY_TIME("After Case 14")
! 2.c.8 Test output
!
!/T      WRITE (NDST,9020)
!/T      DO IP=1, NAPROC
...
!/DIST    END IF
!
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 4'
!/DEBUGINIT     FLUSH(740+IAPROC)
      DEALLOCATE ( NT )
!
! 3.  Model initialization ------------------------------------------- /
...
!
      VA(:,:) = 0.
!/DEBUGMPI     CALL TEST_MPI_STATUS("Case 15")
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 4.0'
!/DEBUGINIT     WRITE(740+IAPROC,*) ' 1: min/max/sum(VA)=', minval(VA), maxval(VA), sum(VA)
!/DEBUGINIT     WRITE(740+IAPROC,*) ' 1: NSEAL=', NSEAL
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB!/DEBUGCOH          CALL ALL_VA_INTEGRAL_PRINT(IMOD, "Before W3IORS call")
...
!/DEBUGINIT     WRITE(740+IAPROC,*) ' 2: min/max/sum(VA)=', minval(VA), maxval(VA), sum(VA)
!/DEBUGINIT     WRITE(740+IAPROC,*) ' 2: NSEAL=', NSEAL
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB!/DEBUGCOH          CALL ALL_VA_INTEGRAL_PRINT(IMOD, "After W3IORS call")
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 4.1'
!/DEBUGINIT     WRITE(740+IAPROC,*) '    sum(VA)=', sum(VA)
!/DEBUGINIT     FLUSH(740+IAPROC)
      ...
      IF ( IAPROC .EQ. NAPLOG ) THEN
          IF (RSTYPE.EQ.0) THEN
            ...
            END IF
        END IF
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 4.2'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB!/DEBUGCOH          CALL ALL_VA_INTEGRAL_PRINT(IMOD, "W3INIT, step 4.2")
!/TIMINGS       CALL PRINT_MY_TIME("After restart inits")
...
!/MEMCHECK      WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_INIT SECTION 3b'
!/MEMCHECK      call getMallocInfo(mallinfos)
!/MEMCHECK      call printMallInfo(IAPROC,mallInfos)
!
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 4.3'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB!/DEBUGCOH          CALL ALL_VA_INTEGRAL_PRINT(IMOD, "W3INIT, step 4.3")
!
! 3.b2 Set MAPSTA associated to PDLIB
!
!/PDLIB      IF (GTYPE .eq. UNGTYPE) THEN
!/PDLIB        CALL PDLIB_MAPSTA_INIT(IMOD)
!/PDLIB      END IF
!
...
!
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 5'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB!/DEBUGCOH          CALL ALL_VA_INTEGRAL_PRINT(IMOD, "W3INIT, step 5")
      IF ( RSTYPE .EQ. 4 ) THEN
          VA(:,:) = 0.
!/T          WRITE (NDST,9031)
        END IF

!/MEMCHECK       WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_INIT SECTION 4'
!/MEMCHECK       call getMallocInfo(mallinfos)
!/MEMCHECK       call printMallInfo(IAPROC,mallInfos)
!
...
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT definition of FSREFR and FRFREQ'
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) 'FSTOTALIMP=', FSTOTALIMP
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) 'FSREFRACTION=', FSREFRACTION
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) 'FSFREQSHIFT=', FSFREQSHIFT
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) 'Before FLCTH=', FLCTH, 'FLCK=', FLCK
!/PDLIB        IF (FSTOTALIMP .and. FSREFRACTION) THEN
!/PDLIB          FLCTH = .FALSE.
!/PDLIB        END IF
!/PDLIB        IF (FSTOTALIMP .and. FSFREQSHIFT) THEN
!/PDLIB          FLCK = .FALSE.
!/PDLIB        END IF
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) ' After FLCTH=', FLCTH, 'FLCK=', FLCK
!
! 4.  Set-up output times -------------------------------------------- *
! 4.a Unpack ODAT
!
...
!
      ...
      DO J=1, NOGRP
        ...
        END DO
      END DO
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 6'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB!/DEBUGCOH          CALL ALL_VA_INTEGRAL_PRINT(IMOD, "W3INIT, step 6")
!
      FLOUT(7) = .FALSE.
      FLOGR2   = FLGR2
      FLOG2    = FLG2
...
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 7'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB!/DEBUGCOH          CALL ALL_VA_INTEGRAL_PRINT(IMOD, "W3INIT, step 7")
!/DEBUGINIT      WRITE(*,*) 'Starting the NOTYPE loop, takes time'
...
!/DEBUGINIT      WRITE(*,*) 'Ending the NOTYPE loop, takes time'
!/TIMINGS       CALL PRINT_MY_TIME("After NOTYPE loop")
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 8'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB!/DEBUGCOH          CALL ALL_VA_INTEGRAL_PRINT(IMOD, "W3INIT, step 8.1")
!
...
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) 'Before INIT_GET_JSEA_ISPROC call'
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) 'allocated(ISEA_TO_JSEA)=', allocated(ISEA_TO_JSEA)
!/PDLIB!/DEBUGINIT     WRITE(740+IAPROC,*) 'NAPROC=', NAPROC
!/PDLIB!/DEBUGINIT     FLUSH(740+IAPROC)
...
!Li     DO IX=1, NX
!Li       ISEA   = MAPFS(IY,IX)
!/DEBUGSTP    WRITE(740+IAPROC,*) 'Debugging the SETUP / WLV'
      DO ISEA=1, NSEA
!/DEBUGSTP    WRITE(740+IAPROC,*) 'ISEA/WLV/ZB=', ISEA, WLV(ISEA), ZB(ISEA)
        IX = MAPSF(ISEA,1)
        IY = MAPSF(ISEA,2)
...
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 9'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/PDLIB!/DEBUGCOH          CALL ALL_VA_INTEGRAL_PRINT(IMOD, "W3INIT, step 8.2")

!
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 9.1'
!/DEBUGINIT     WRITE(740+IAPROC,*) ' allocated(MAPTST)=', allocated(MAPTST)
!/DEBUGINIT     WRITE(740+IAPROC,*) 'NY=', NY, ' NX=', NX
!/DEBUGINIT     FLUSH(740+IAPROC)
      MAPST2 = MAPST2 + 2*MAPTST
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 9.2'
!/DEBUGINIT     FLUSH(740+IAPROC)
!
      DEALLOCATE ( MAPTST )
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 9.3'
!/DEBUGINIT     FLUSH(740+IAPROC)

!/MEMCHECK       WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_INIT SECTION 6'
!/MEMCHECK       call getMallocInfo(mallinfos)
!/MEMCHECK       call printMallInfo(IAPROC,mallInfos)
!
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 9.4'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/T      WRITE (NDST,9050)
!/T      NX0    = 1
!/T      DO
...
!/TIMINGS       CALL PRINT_MY_TIME("Before section 5.b")
!
...
!
      AS    (0) = 0.
      DW    (0) = 0.
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 9.7'
!/DEBUGINIT     FLUSH(740+IAPROC)
!
! 7.  Write info to log file ----------------------------------------- /
!
...
!
        END IF
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 9.8'
!/DEBUGINIT     FLUSH(740+IAPROC)
!
      IF ( NOPTS .EQ. 0 ) FLOUT(2) = .FALSE.

!/MEMCHECK       WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_INIT SECTION 7'
!/MEMCHECK       call getMallocInfo(mallinfos)
!/MEMCHECK       call printMallInfo(IAPROC,mallInfos)
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 9.9'
!/DEBUGINIT     FLUSH(740+IAPROC)
!
! Boundary set up for the directions
!
!/PDLIB!/DEBUGCOH          CALL ALL_VA_INTEGRAL_PRINT(IMOD, "W3INIT, step 8.3")
!!/PDLIB         CALL VA_SETUP_IOBPD
!/PDLIB!/DEBUGCOH          CALL ALL_VA_INTEGRAL_PRINT(IMOD, "W3INIT, step 8.4")
!/DEBUGINIT     WRITE(740+IAPROC,*) 'W3INIT, aft BLOCK_SOLVER_INIT, step 9.10'
!/DEBUGINIT     FLUSH(740+IAPROC)
!
! 8.  Final MPI set up ----------------------------------------------- /
!
!/MPI      CALL W3MPII ( IMOD )
!/DEBUGINIT     WRITE(740+IAPROC,*) 'After W3MPII'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/MPI      CALL W3MPIO ( IMOD )
!/DEBUGINIT     WRITE(740+IAPROC,*) 'After W3MPIO'
!/DEBUGINIT     FLUSH(740+IAPROC)
!/MPI      IF ( FLOUT(2) ) CALL W3MPIP ( IMOD )
!/DEBUGINIT     WRITE(740+IAPROC,*) 'After W3MPIP'
!/DEBUGINIT     FLUSH(740+IAPROC)
!
!/PDLIB!/DEBUGINIT         CALL PRINT_WN_STATISTIC("W3INIT leaving")
!/TIMINGS       CALL PRINT_MY_TIME("Leaving W3INIT")
      RETURN
!
...
!
!/DEBUGINIT      WRITE(740+IAPROC,*) 'W3MPII, step 1'
!/DEBUGINIT      FLUSH(740+IAPROC)
      NXXXX  = NSEALM * NAPROC
!
...
!/DEBUGINIT          WRITE(*,*) 'Before VA MPI_SEND/RECV_INIT inits'
!/MPI      DO ISP=1, NSPEC
...
!/MPI          END IF
!/MPI        END DO
!/DEBUGINIT           WRITE(*,*) 'After VA MPI_SEND/RECV_INIT inits'
...
!/MPI      USE W3IORSMD, ONLY: OARST
      USE W3SERVMD, ONLY: EXTCDE
!/S      USE W3SERVMD, ONLY: STRACE
!/
      USE W3GDATMD, ONLY: NSEA
      USE W3ADATMD, ONLY: NSEALM
!/MPI      USE W3GDATMD, ONLY: NX, NSPEC, MAPFS, E3DF, P2MSF, US3DF, USSPF
...
!/MPI                          FLOGR2
!/MPI      USE W3PARALL, ONLY : INIT_GET_JSEA_ISPROC
      USE W3GDATMD, ONLY: GTYPE, UNGTYPE
      USE CONSTANTS, ONLY: LPDLIB
!/
      IMPLICIT NONE
...
!/MPI      LOGICAL                 :: FLGRDARST(NOGRP,NGRPP)
...
!/MPI          FLGRDARST(J,K) =  (FLGRDALL(J,K) .OR. FLOGRR(J,K))
!/MPI          END DO
!/MPI        END DO
!
!/MPI      NRQGO  = 0
...
!
!/MPI          ! Calculation of NRQMAX splitted by output groups and field type
!/MPI          !       scalar                2-comp   3-comp
!/MPI          NRQMAX =   1                +    0  +    0  +  &  ! group 1 
!/MPI                    18                +    0  +    0  +  &  ! group 2
!/MPI                     0                +    0  +    0  +  &  ! group 3 (extra contributions below)
!/MPI             2+(NOGE(4)-2)*(NOSWLL+1) +    0  +    0  +  &  ! group 4
!/MPI                    11                +    3  +    1  +  &  ! group 5
!/MPI                    12                +    7  +    1  +  &  ! group 6 (extra contributions below)
!/MPI                     5                +    4  +    1  +  &  ! group 7
!/MPI                     5                +    2  +    0  +  &  ! group 8
!/MPI                     5                +    0  +    0  +  &  ! group 9
!/MPI                NOEXTR                +    0  +    0        ! group 10
!/MPI
!/MPI          ! Extra contributions to NRQMAX from group 3
!/MPI          DO IFJ=1,5
!/MPI            IF ( FLGRDALL( 3,IFJ)) NRQMAX = NRQMAX +               &
!/MPI                                    E3DF(3,IFJ) - E3DF(2,IFJ) + 1
!/MPI            END DO
!/MPI          ! Extra contributions to NRQMAX from group 6
...
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 2, 6) .OR. FLGRDALL( 2,18) ) THEN
!/MPI                  ! TP output shares FP0 internal field with FP
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
...
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 2, 17) ) THEN
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (WBT  (1),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 2/17', IROOT, IT, IRQGO(IH), IERR
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 2, 19) ) THEN
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (WNMEAN(1),NSEALM , MPI_REAL, IROOT,   &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 2/19', IROOT, IT, IRQGO(IH), IERR
!/MPI                END IF
!
...
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (PTHP0(1,K),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
...
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (PQP (1,K),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 4/08', IROOT, IT, IRQGO(IH), IERR
!/MPI                  END DO
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 4, 9) ) THEN
!/MPI                DO K=0, NOSWLL
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (PPE (1,K),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 4/09', IROOT, IT, IRQGO(IH), IERR
!/MPI                  END DO
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 4,10) ) THEN
!/MPI                DO K=0, NOSWLL
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (PGW (1,K),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 4/10', IROOT, IT, IRQGO(IH), IERR
!/MPI                  END DO
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 4,11) ) THEN
!/MPI                DO K=0, NOSWLL
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (PSW (1,K),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 4/11', IROOT, IT, IRQGO(IH), IERR
!/MPI                  END DO
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 4,12) ) THEN
!/MPI                DO K=0, NOSWLL
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (PTM1(1,K),NSEALM , MPI_REAL, IROOT,   &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 4/12', IROOT, IT, IRQGO(IH), IERR
!/MPI                  END DO
!/MPI                END IF
!
!
!/MPI              IF ( FLGRDALL( 4,13) ) THEN
!/MPI                DO K=0, NOSWLL
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (PT1 (1,K),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 4/13', IROOT, IT, IRQGO(IH), IERR
!/MPI                  END DO
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 4,14) ) THEN
!/MPI                DO K=0, NOSWLL
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (PT2 (1,K),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 4/14', IROOT, IT, IRQGO(IH), IERR
!/MPI                  END DO
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 4,15) ) THEN
!/MPI                DO K=0, NOSWLL
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (PEP (1,K),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 4/15', IROOT, IT, IRQGO(IH), IERR
!/MPI                  END DO
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 4,16) ) THEN
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (PWST (1),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 4/16', IROOT, IT, IRQGO(IH), IERR
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 4,17) ) THEN
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (PNR  (1),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 4/17', IROOT, IT, IRQGO(IH), IERR
!/MPI                END IF
!
...
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 5, 11) ) THEN
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (TWS(1),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 5/11', IROOT, IT, IRQGO(IH), IERR
!/MPI                END IF
!
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (USSP(1,IK),NSEALM , MPI_REAL, IROOT,  &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
...
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (TAUOCX(1),NSEALM , MPI_REAL, IROOT,   &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 6/13', IROOT, IT, IRQGO(IH), IERR
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (TAUOCY(1),NSEALM , MPI_REAL, IROOT,   &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 6/13', IROOT, IT, IRQGO(IH), IERR
!/MPI                END IF
!
...
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 8, 3) ) THEN
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (MSSD  (1),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 8/03', IROOT, IT, IRQGO(IH), IERR
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 8, 4) ) THEN
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (MSCD  (1),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 8/04', IROOT, IT, IRQGO(IH), IERR
!/MPI                END IF
!
!/MPI              IF ( FLGRDALL( 8, 5) ) THEN
!/MPI                  IH     = IH + 1
!/MPI                  IT     = IT + 1
!/MPI      CALL MPI_SEND_INIT (QP    (1),NSEALM , MPI_REAL, IROOT,    &
!/MPI                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
!/MPIT      WRITE (NDST,9011) IH, ' 8/05', IROOT, IT, IRQGO(IH), IERR
!/MPI                END IF
!
...
!
!/MPI              IF (NAPFLD .EQ. NAPRST) THEN
!/MPI                CALL W3XDMA ( IMOD, NDSE, NDST, FLGRDARST )
!/MPI              ELSE
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (WBT(I0),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (WNMEAN(I0),1,WW3_FIELD_VEC, IFROM, IT, &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 2/19', IFROM, IT, IRQGO2(IH), IERR
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 3, 1) ) THEN 
!/MPI                    DO IK=E3DF(2,1),E3DF(3,1)
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (PTHP0(I0,K),1,WW3_FIELD_VEC, IFROM, IT,&
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (PQP(I0,K),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (PPE(I0,K),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 4/09', IFROM, IT, IRQGO2(IH), IERR
!/MPI                    END DO
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 4,10) ) THEN
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (PGW(I0,K),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 4/10', IFROM, IT, IRQGO2(IH), IERR
!/MPI                    END DO
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 4,11) ) THEN
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (PSW(I0,K),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 4/11', IFROM, IT, IRQGO2(IH), IERR
!/MPI                    END DO
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 4,12) ) THEN
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (PTM1(I0,K),1,WW3_FIELD_VEC, IFROM, IT,&
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 4/12', IFROM, IT, IRQGO2(IH), IERR
!/MPI                    END DO
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 4,13) ) THEN
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (PT1(I0,K),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 4/13', IFROM, IT, IRQGO2(IH), IERR
!/MPI                    END DO
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 4,14) ) THEN
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (PT2(I0,K),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 4/14', IFROM, IT, IRQGO2(IH), IERR
!/MPI                    END DO
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 4,15) ) THEN
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (PEP(I0,K),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 4/15', IFROM, IT, IRQGO2(IH), IERR
!/MPI                    END DO
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 4,16) ) THEN
...
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 4/16', IFROM, IT, IRQGO2(IH), IERR
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 4,17) ) THEN
!/MPI                    IH     = IH + 1
...
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 4/17', IFROM, IT, IRQGO2(IH), IERR
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 5, 1) ) THEN
!/MPI                    IH     = IH + 1
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (TWS(I0),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 5/11', IFROM, IT, IRQGO2(IH), IERR
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 6, 1) ) THEN
!/MPI                    IH     = IH + 1
...
!/MPI                      IH     = IH + 1
!/MPI                      IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (USSP(I0,IK),1,WW3_FIELD_VEC, IFROM, IT, &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (TAUOCX(I0),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 6/13', IFROM, IT, IRQGO2(IH), IERR
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (TAUOCY(I0),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 6/13', IFROM, IT, IRQGO2(IH), IERR
!/MPI                  END IF
!
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (MSSD  (I0),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 8/03', IFROM, IT, IRQGO2(IH), IERR
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 8, 4) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (MSCD (I0),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 8/04', IFROM, IT, IRQGO2(IH), IERR
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 8, 5) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT + 1
!/MPI      CALL MPI_RECV_INIT (QP   (I0),1,WW3_FIELD_VEC, IFROM, IT,  &
!/MPI                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
!/MPIT      WRITE (NDST,9011) IH, ' 8/05', IFROM, IT, IRQGO2(IH), IERR
!/MPI                  END IF
!
!/MPI                IF ( FLGRDALL( 9, 1) ) THEN
!/MPI                    IH     = IH + 1
...
!
!/MPI                DO I=1, NOEXTR
!/MPI                  !WRITE(740+IAPROC,*) 'SECOND : I=', I, ' / ', NOEXTR, ' val=', FLGRDALL(10, I)
!/MPI                  IF ( FLGRDALL(10, I) ) THEN
!/MPI                      IH     = IH + 1
!/MPI                      IT     = IT + 1
...
!/MPI          IF (OARST) THEN
!/MPI            ALLOCATE ( OUTPTS(IMOD)%OUT4%IRQRS(34*NAPROC) )
!/MPI          ELSE
...
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 4
!/MPI                CALL MPI_SEND_INIT (CX(IAPROC), 1, WW3_FIELD_VEC,   &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S CX', IROOT, IT, IRQRS(IH), IERR
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 5
!/MPI                CALL MPI_SEND_INIT (CY(IAPROC), 1, WW3_FIELD_VEC,   &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S CY', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 1, 9) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 6
!/MPI                CALL MPI_SEND_INIT (ICEF(IAPROC), 1, WW3_FIELD_VEC, &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S IF', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 2, 1) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 7
!/MPI                CALL MPI_SEND_INIT (HS   (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S HS', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 2, 2) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 8
!/MPI                CALL MPI_SEND_INIT (WLM  (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S WL', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 2, 4) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 9
!/MPI                CALL MPI_SEND_INIT (T0M1(1), NSEALM, MPI_REAL,      &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S T0', IROOT, IT, IRQRS(IH), IERR
!/MPI              ENDIF
!
!/MPI              IF ( FLOGRR( 2, 5) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 10
!/MPI                CALL MPI_SEND_INIT (T01 (1), NSEALM, MPI_REAL,      &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S T1', IROOT, IT, IRQRS(IH), IERR
!/MPI              ENDIF
!
!/MPI              IF ( FLOGRR( 2, 6) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 11
!/MPI                CALL MPI_SEND_INIT (FP0  (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S FP', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 2, 7) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 12
!/MPI                CALL MPI_SEND_INIT (THM  (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S TH', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 2, 19) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 13
!/MPI                CALL MPI_SEND_INIT (WNMEAN(1), NSEALM, MPI_REAL,    &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S WM', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 5, 2) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 14
!/MPI                CALL MPI_SEND_INIT (CHARN(1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S CH', IROOT, IT, IRQRS(IH), IERR
!/MPI              ENDIF
!
!/MPI              IF ( FLOGRR( 5, 5) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 15
!/MPI                CALL MPI_SEND_INIT (TAUWIX(1), NSEALM, MPI_REAL,    &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S WX', IROOT, IT, IRQRS(IH), IERR
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 16
!/MPI                CALL MPI_SEND_INIT (TAUWIY(1), NSEALM, MPI_REAL,    &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S WY', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 5, 11) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 17
!/MPI                CALL MPI_SEND_INIT (TWS  (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S TS', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 6, 2) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 18
!/MPI                CALL MPI_SEND_INIT (TAUOX(1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S OX', IROOT, IT, IRQRS(IH), IERR
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 19
!/MPI                CALL MPI_SEND_INIT (TAUOY(1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S OY', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 6, 3) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 20
!/MPI                CALL MPI_SEND_INIT (BHD  (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S BH', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 6, 4) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 21
!/MPI                CALL MPI_SEND_INIT (PHIOC(1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S PH', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 6, 5) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 22
!/MPI                CALL MPI_SEND_INIT (TUSX (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S UX', IROOT, IT, IRQRS(IH), IERR
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 23
!/MPI                CALL MPI_SEND_INIT (TUSY (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S UY', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 6, 6) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 24
!/MPI                CALL MPI_SEND_INIT (USSX (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S SX', IROOT, IT, IRQRS(IH), IERR
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 25
!/MPI                CALL MPI_SEND_INIT (USSY (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S SY', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 6,10) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 26
!/MPI                CALL MPI_SEND_INIT (TAUICE(1,1), NSEALM, MPI_REAL,  &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S I1', IROOT, IT, IRQRS(IH), IERR
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 27
!/MPI                CALL MPI_SEND_INIT (TAUICE(1,2), NSEALM, MPI_REAL,  &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S I2', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 6,13) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 28
!/MPI                CALL MPI_SEND_INIT (TAUOCX(1), NSEALM, MPI_REAL,    &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S TX', IROOT, IT, IRQRS(IH), IERR
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 29
!/MPI                CALL MPI_SEND_INIT (TAUOCY(1), NSEALM, MPI_REAL,    &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S TY', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 7, 2) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 30
!/MPI                CALL MPI_SEND_INIT (UBA  (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S BA', IROOT, IT, IRQRS(IH), IERR
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 31
!/MPI                CALL MPI_SEND_INIT (UBD  (1), NSEALM, MPI_REAL,     &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S BD', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 7, 4) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 32
!/MPI                CALL MPI_SEND_INIT (PHIBBL(1), NSEALM, MPI_REAL,    &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
!/MPIT        WRITE (NDST,9021) IH, 'S PB', IROOT, IT, IRQRS(IH), IERR
!/MPI              END IF
!
!/MPI              IF ( FLOGRR( 7, 5) ) THEN
!/MPI                IH     = IH + 1
!/MPI                IT     = IT0 + 33
!/MPI                CALL MPI_SEND_INIT (TAUBBL(1,1), NSEALM, MPI_REAL,  &
!/MPI                         IROOT, IT, MPI_COMM_WAVE, IRQRS(IH), IERR)
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 4
!/MPI                    CALL MPI_RECV_INIT (CX   (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R CX', IFROM, IT, IRQRS(IH), IERR
!/MPI                    IH     = IT0 + 5
!/MPI                    IT     = IT + 1
!/MPI                    CALL MPI_RECV_INIT (CY   (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R CY', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 1, 9) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 6
!/MPI                    CALL MPI_RECV_INIT (ICEF (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R IF', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 2, 1) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 7
!/MPI                    CALL MPI_RECV_INIT (HS   (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R HS', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 2, 2) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 8
!/MPI                    CALL MPI_RECV_INIT (WLM  (I0),1,WW3_FIELD_VEC,  &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R WL', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 2, 4) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 9
!/MPI                    CALL MPI_RECV_INIT (T0M1(I0),1,WW3_FIELD_VEC,  &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R T0', IFROM, IT, IRQRS(IH), IERR
!/MPI                  ENDIF
!
!/MPI                  IF ( FLOGRR( 2, 5) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 10
!/MPI                    CALL MPI_RECV_INIT (T01 (I0),1,WW3_FIELD_VEC,  &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R T1', IFROM, IT, IRQRS(IH), IERR
!/MPI                  ENDIF
!
!/MPI                  IF ( FLOGRR( 2, 6) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 11
!/MPI                    CALL MPI_RECV_INIT (FP0  (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R FP', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 12
!/MPI                    CALL MPI_RECV_INIT (THM  (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R TH', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 2, 19) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 13
!/MPI                    CALL MPI_RECV_INIT (WNMEAN(I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R WM', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 5, 2) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 14
!/MPI                    CALL MPI_RECV_INIT (CHARN(I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R CH', IFROM, IT, IRQRS(IH), IERR
!/MPI                  ENDIF
!
!/MPI                  IF ( FLOGRR( 5, 5) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 15
!/MPI                    CALL MPI_RECV_INIT (TAUWIX(I0),1,WW3_FIELD_VEC,&
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R WX', IFROM, IT, IRQRS(IH), IERR
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 16
!/MPI                    CALL MPI_RECV_INIT (TAUWIY(I0),1,WW3_FIELD_VEC,&
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R WY', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 5,11) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 17
!/MPI                    CALL MPI_RECV_INIT (TWS  (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R TS', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 6, 2) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 18
!/MPI                    CALL MPI_RECV_INIT (TAUOX(I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R OX', IFROM, IT, IRQRS(IH), IERR
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 19
!/MPI                    CALL MPI_RECV_INIT (TAUOY(I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R OY', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 6, 3) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 20
!/MPI                    CALL MPI_RECV_INIT (BHD  (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R BH', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 6, 4) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 21
!/MPI                    CALL MPI_RECV_INIT (PHIOC(I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R PH', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 6, 5) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 22
!/MPI                    CALL MPI_RECV_INIT (TUSX (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R UX', IFROM, IT, IRQRS(IH), IERR
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 23
!/MPI                    CALL MPI_RECV_INIT (TUSY (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R UY', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 6, 6) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 24
!/MPI                    CALL MPI_RECV_INIT (USSX (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R SX', IFROM, IT, IRQRS(IH), IERR
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 25
!/MPI                    CALL MPI_RECV_INIT (USSY (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R SY', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 6,10) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 26
!/MPI                    CALL MPI_RECV_INIT (TAUICE(I0,1),1,WW3_FIELD_VEC,&
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R I1', IFROM, IT, IRQRS(IH), IERR
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 27
!/MPI                    CALL MPI_RECV_INIT (TAUICE(I0,2),1,WW3_FIELD_VEC,&
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R I2', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 6,13) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 28
!/MPI                    CALL MPI_RECV_INIT (TAUOCX(I0),1,WW3_FIELD_VEC,&
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R SX', IFROM, IT, IRQRS(IH), IERR
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 29
!/MPI                    CALL MPI_RECV_INIT (TAUOCY(I0),1,WW3_FIELD_VEC,&
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R SY', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
...
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 30
!/MPI                    CALL MPI_RECV_INIT (UBA  (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R BA', IFROM, IT, IRQRS(IH), IERR
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 31
!/MPI                    CALL MPI_RECV_INIT (UBD  (I0),1,WW3_FIELD_VEC, &
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R BD', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 7, 4) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 32
!/MPI                    CALL MPI_RECV_INIT (PHIBBL(I0),1,WW3_FIELD_VEC,&
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R PB', IFROM, IT, IRQRS(IH), IERR
!/MPI                  END IF
!
!/MPI                  IF ( FLOGRR( 7, 5) ) THEN
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 33
!/MPI                    CALL MPI_RECV_INIT (TAUBBL(I0,1),1,WW3_FIELD_VEC,&
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
!/MPIT      WRITE (NDST,9021) IH, 'R T1', IFROM, IT, IRQRS(IH), IERR
!/MPI                    IH     = IH + 1
!/MPI                    IT     = IT0 + 34
!/MPI                    CALL MPI_RECV_INIT (TAUBBL(I0,2),1,WW3_FIELD_VEC,&
!/MPI                       IFROM, IT, MPI_COMM_WAVE, IRQRS(IH), IERR )
...
!/MPI              IP(J)  = NAPPNT
!/MPI            ELSE
!/MPI              CALL INIT_GET_JSEA_ISPROC(IS(J), JSEA, IP(J))
!/MPI            END IF
!
