...
!/    22-Mar-2021 : Add new coupling fields             ( version 7.xx )
!/
...
#ifdef W3_NCC
      CFLAG = .FALSE. 
      CALL WW_INIT(1,GRIDS(1)%NX,GRIDS(1)%NY,            &
      GRIDS(1)%X0,GRIDS(1)%Y0,GRIDS(1)%SX,GRIDS(1)%SY)
      CALL WW_RECVDTC
      CALL WW_SENDGRIDS
      CALL WW_SENDSLM(GRIDS(1)%MAPSTA)
      IF ( IAPROC .EQ. NAPOUT .AND. DEBUG_NCC ) THEN
         OPEN(9751,FILE='debug_writeout_wind',           &
              FORM='UNFORMATTED',STATUS='UNKNOWN')
         OPEN(9752,FILE='debug_writeout_ice',            &
              FORM='UNFORMATTED',STATUS='UNKNOWN')
         OPEN(9753,FILE='data_forDC',                    &
              FORM='UNFORMATTED',STATUS='UNKNOWN')
         WRITE(9753) GRIDS(1)%NX,GRIDS(1)%NY,            &
           GRIDS(1)%X0,GRIDS(1)%Y0,GRIDS(1)%SX,GRIDS(1)%SY
         WRITE(9753) GRIDS(1)%MAPSTA
      END IF
#endif // W3_NCC
...
!
#ifdef W3_NCC
      IF ( IAPROC .EQ. NAPOUT .AND. DEBUG_NCC ) WRITE(9753) TIME0
      CALL WW_TSTEP_INIT(TIME0)
      CALL WW_RECV_SBC
      CALL WW_RECV_PSEUDOICE
#endif // W3_NCC
!
...
#ifdef W3_NCC
                END IF
                CALL WW_CDETECT(CFLAG(J))
                IF ( IAPROC .EQ. NAPOUT .AND. DEBUG_NCC )     &
                     WRITE(9753) TW0, TWN
                CALL WW_UPDATE_WIND(TW0,TWN,WX0,WY0,WXN,WYN)
                IF ( IAPROC .EQ. NAPOUT .AND. DEBUG_NCC )     &
                     WRITE(9751) TW0, TWN, WX0, WY0, WXN, WYN
#endif // W3_NCC
              END IF
...
!could be:      IF ( IERR .LT. 0 ) FLLST_ALL(J) = .TRUE.
                ...
                IF ( IAPROC .EQ. NAPOUT .AND. DEBUG_NCC )    &
                     WRITE(9753) TIN
                CALL WW_UPDATE_PSEUDOICE(TIN,ICEI)
                IF ( IAPROC .EQ. NAPOUT .AND. DEBUG_NCC )    &
                     WRITE(9752) TIN, ICEI
#endif // W3_NCC
              END IF
