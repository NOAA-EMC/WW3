...
!/
!/SMC  !Li   Temperature spectra for Arctic boundary update.
!/SMC      REAL, ALLOCATABLE       :: BACSPEC(:)
!/SMC      REAL                    :: BACANGL
!/SMC
...
!
      IF( GTYPE .EQ. SMCTYPE ) THEN
             J = 1
!/SMC !!Li   Use sea point only field for SMC grid.
...
!/T          WRITE (NDST,9017) DTTST1, DTTST2, DTTST3
          IF ( DTTST1.LT.0. .OR. DTTST2.LT.0. .OR. DTTST3.LT.0. ) THEN
...
!/IS2                      IF ( DTTST .LE. 0.5*DTI50 ) IDACT(18:18) = 'U'
!/IS2                    END IF
!/IS2                ELSE
!/IS2                  IDACT(18:18) = 'I'
!/IS2                END IF
!
!/IS2              IF ( IDACT(18:18).NE.' ' ) THEN
...
!/PDLIB                   TAUOCX(JSEA), TAUOCY(JSEA), WNMEAN(JSEA),      &
!/PDLIB                   RHOAIR(ISEA), ASF(ISEA))
...
!
                IF( GTYPE .EQ. SMCTYPE ) THEN
                    IX = 1
!/SMC !!Li   Use SMC sub to gether field
...
!
                IF( GTYPE .EQ. SMCTYPE ) THEN
                  IX = 1
!/SMC !!Li   Use SMC sub to scatter field
...
!Li   Initialise IK IX IY in case ARC option is not used to avoid warnings.
              ...
              IX=1
              IY=1
!/SMC  !Li    Find source boundary spectra and assign to SPCBAC
!/SMC        IF( ARCTC ) THEN
!/SMC 
!/SMC        DO IK = 1, NBAC
!/SMC           IF( IK .LE. (NBAC-NBGL) ) THEN
!/SMC                   IY = ICLBAC(IK)
!/SMC           ELSE
!/SMC                   IY = NGLO + IK 
!/SMC           ENDIF 
!/SMC 
!/SMC  !Li    Work out root PE (ISPEC) and JSEA numbers for IY 
!/SMC/!/DIST            ISPEC = MOD( IY-1, NAPROC ) 
!/SMC/!/DIST             JSEA = 1 + (IY - ISPEC - 1)/NAPROC
!/SMC/!/SHRD            ISPEC = 0 
!/SMC/!/SHRD             JSEA = IY 
!
!/SMC !!Li   Assign boundary cell spectra. 
!/SMC              IF( IAPROC .EQ. ISPEC+1 ) THEN
!/SMC                   SPCBAC(:,IK)=VA(:,JSEA)
!/SMC              ENDIF
!
!/SMC !!Li   Broadcast local SPCBAC(:,IK) to all other PEs.
!/SMC/!/MPI         CALL MPI_BCAST(SPCBAC(1,IK),NSPEC,MPI_REAL,ISPEC,MPI_COMM_WAVE,IERR_MPI)
!/SMC/!/MPI         CALL MPI_BARRIER (MPI_COMM_WAVE,IERR_MPI)
!
!/SMC        END DO   !! Loop IK ends.
!
!/SMC !!Li    Update Arctic boundary cell spectra if within local range
!/SMC           ALLOCATE ( BACSPEC(NSPEC) )
!/SMC        DO IK = 1, NBAC
!/SMC           IF( IK .LE. (NBAC-NBGL) ) THEN
!/SMC                   IX = NGLO + IK 
!/SMC                   BACANGL = ANGARC(IK)
!/SMC           ELSE
!/SMC                   IX = ICLBAC(IK)
!/SMC                   BACANGL = - ANGARC(IK)
!/SMC           ENDIF 
!/SMC
!/SMC !!Li    Work out boundary PE (ISPEC) and JSEA numbers for IX 
!/SMC/!/DIST            ISPEC = MOD( IX-1, NAPROC ) 
!/SMC/!/DIST             JSEA = 1 + (IX - ISPEC - 1)/NAPROC
!/SMC/!/SHRD            ISPEC = 0 
!/SMC/!/SHRD             JSEA = IX 
!
!/SMC              IF( IAPROC .EQ. ISPEC+1 ) THEN
!/SMC                   BACSPEC = SPCBAC(:,IK)
!/SMC                  
!/SMC                   CALL w3acturn( NTH, NK, BACANGL, BACSPEC )
!/SMC
!/SMC                   VA(:,JSEA) = BACSPEC 
!/SMC !!Li              WRITE(NDSE,*) "IAPROC, IX, JSEAx, IK=", IAPROC, IX, JSEA, IK 
!/SMC              ENDIF
!/SMC
!/SMC        END DO  !! Loop IK ends.
!/SMC        DEALLOCATE ( BACSPEC )
!/SMC
!/SMC        ENDIF  !! ARCTC
!
...
!/T 9017 FORMAT (' TEST W3WAVE : DT TAU  =',F12.1)
!/T 9018 FORMAT (' TEST W3WAVE : DT RHO  =',F12.1)
