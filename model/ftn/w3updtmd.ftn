...
!/SMC      USE W3GDATMD, ONLY: NARC, NGLO, ANGARC 
!/SMC      USE W3GDATMD, ONLY: FSWND, ARCTC 
...
!/SMC  !Li   Rotate curreent direction by ANGARC for Arctic part cells.  JGLi23Mar2016
!/SMC        IF( ARCTC .AND. (ISEA .GT. NGLO) ) THEN
!/SMC            DN = CDIR + ANGARC( ISEA - NGLO )*DERA
!/SMC            CDIR = MOD ( TPI + DN, TPI )
!/SMC        ENDIF
...
!/SMC      USE W3GDATMD, ONLY: NARC, NGLO, ANGARC, ARCTC, FSWND 
...
!/SMC  !Li   Rotate wind direction by ANGARC for Arctic part cells.
!/SMC        IF( ARCTC .AND. (ISEA .GT. NGLO) ) THEN
...
!/ ------------------------------------------------------------------- /
!/
!/S      CALL STRACE (IENT, 'W3UTAU')
!
! 1.  Prepare auxiliary arrays
!
      IF ( FLFRST ) THEN
          DO ISEA=1, NSEA
!/SMC !!Li  For sea-point only SMC grid momentum 1-D momentum is stored on
!/SMC !!Li  2-D UX0(NSEA, 1) variable.
!/SMC        IF( FSWND ) THEN
!/SMC            IX = ISEA 
!/SMC            IY = 1
!/SMC        ELSE
...
!
!/OMPG/!$OMP PARALLEL DO PRIVATE (ISEA,MI2,MXR,MYR,MDARC)
!
      DO ISEA=1, NSEA
!
        TAUA(ISEA) = MA0(ISEA) + RD * MAI(ISEA)
!/WNT2        MI2      = SQRT ( RD2 *      MA0(ISEA)**2 +             &
!/WNT2                          RD  *(MA0(ISEA)+MAI(ISEA))**2 )
!/WNT2        TAUA(ISEA) = TAUA(ISEA) * MIN(1.25,MI2/MAX(1.E-7,TAUA(ISEA)))
        TAUADIR(ISEA) = MD0(ISEA) + RD * MDI(ISEA)
!/SMC  !Li   Rotate momentum direction by ANGARC for Arctic part cells.
!/SMC        IF( ARCTC .AND. (ISEA .GT. NGLO) ) THEN
!/SMC            MDARC = TAUADIR(ISEA) + ANGARC( ISEA - NGLO )*DERA
!/SMC            TAUADIR(ISEA) = MOD ( TPI + MDARC, TPI )
!/SMC        ENDIF
!
        END DO
!
...
! Formats
!
!/T 9000 FORMAT (' TEST W3UTAU : DT0N, DT0T, RD :',2F8.1,F6.3)
!/
!/ End of W3UTAU ----------------------------------------------------- /
!/
...
!/ ------------------------------------------------------------------- /
!/
!/S      CALL STRACE (IENT, 'W3URHO')
!
! 1.  Prepare auxiliary arrays
!
      IF ( FLFRST ) THEN
          DO ISEA=1, NSEA
!/SMC !!Li  For sea-point only SMC grid air density is stored on
!/SMC !!Li  2-D RH0(NSEA, 1) variable.
!/SMC        IF( FSWND ) THEN
!/SMC            IX = ISEA 
!/SMC            IY = 1
!/SMC        ELSE
...
! 3.  Actual momentum for all grid points
!
!/OMPG/!$OMP PARALLEL DO PRIVATE (ISEA,RA0,RAI)
!
      DO ISEA=1, NSEA
!
...
! Formats
!
!/T 9000 FORMAT (' TEST W3URHO : DT0N, DT0T, RD :',2F8.1,F6.3)
!/
!/ End of W3URHO ----------------------------------------------------- /
!/
