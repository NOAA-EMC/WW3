...
!/T      WRITE (MDST,9010)
!
!/SMC !!  Check GTYPE for all grids.
!/SMC       IF( IMPROC.EQ.NMPERR )  WRITE(MDSE,*) " GTYPES in WMGLOW:", &
!/SMC           ( GRIDS(I)%GTYPE, I=1, NRGRD )
!
! -------------------------------------------------------------------- /
! 2.  Process grids
!
...
!/T            WRITE (MDST,9022) 'RANK = 1, DATA FROM FILE'
          ...
          END IF
!
!/SMC !!  SMC grid only appears in same ranked group.  JGLi23Mar2021
!/SMC       IF( GRIDS(I)%GTYPE .EQ. SMCTYPE ) THEN
!/SMC         IF( IMPROC.EQ.NMPERR ) WRITE(MDSE,*) ' WMGLOW skip SMC grid', I 
!/SMC         CYCLE
!/SMC       END IF
!
...
!
!/SMC !!  SMC grid only suppots same ranked group so far.  JGLi12Apr2021
!/SMC             IF( GRIDS(J)%GTYPE .EQ. SMCTYPE ) THEN
!/SMC                 IF( IMPROC.EQ.NMPERR ) WRITE(MDSE,*)   &
!/SMC                   ' WMGLOW skip SMC grid', J 
!/SMC                 CYCLE
!/SMC             END IF
!
! ... Check if in grid
...
!
!/SMC !!  Check GTYPE for all grids.
!/SMC       IF( IMPROC.EQ.NMPERR )  WRITE (MDSE,*) " WMGEQL GTYPE:",   &
!/SMC           ( GRIDS(I)%GTYPE, I=1, NRGRD )
!
! -------------------------------------------------------------------- /
...
!
        NR     = 0
!
!/SMC !!  SMC grids use WMSMCEQL for equal ranked grids.  JGLi23Mar2021
!/SMC        IF( GRIDS(I)%GTYPE .EQ. SMCTYPE ) CYCLE
!
        DO J=1, NRGRD

          IF ( GRANK(I).NE.GRANK(J) .OR. I.EQ.J ) CYCLE
!
!/SMC        IF( GRIDS(J)%GTYPE .EQ. SMCTYPE ) CYCLE
!
!/T          WRITE (MDST,9012) J
          SHRANK(I,J) = .TRUE.
          NR          = NR + 1
...
!
      USE W3SERVMD, ONLY: EXTCDE
!/SMC      USE W3PSMCMD, ONLY: W3SMCGMP, W3SMCELL
...
!/MPI      INTEGER, Dimension(MPI_STATUS_SIZE):: MPIState
...
!/MPI          IF( IMPROC .EQ. ICROOT )  THEN
            WRITE(MDSE,*) "ICROOT, NT are", ICROOT, NT
!/MPI          ENDIF
!
! 1.c.2 SMC grid I boundary cell ids are saved in NBSMC.
!/SMC       ELSEIF( GRIDS(I)%GTYPE .EQ. SMCTYPE ) THEN
!/MPI/!/SMC          IF( IMPROC .EQ. ICROOT )  THEN
!/SMC                  NT = GRIDS(I)%NBSMC    
!/SMC                  WRITE(MDSE,*) "ICROOT, NT are", ICROOT, NT
!/MPI/!/SMC          ENDIF

!/MPI/!/SMC          CALL MPI_BCAST( NT, 1, MPI_INTEGER, &
!/MPI/!/SMC                   ICROOT-1, MPI_COMM_MWAVE, IEER) 

!  Need to wait for all PEs get these values.
!/MPI/!/SMC          CALL MPI_BARRIER (MPI_COMM_MWAVE,IEER)
!
...
!/SMC       ELSEIF( GRIDS(I)%GTYPE .EQ. SMCTYPE ) THEN
!/SMC
!/SMC              IBPTS = GRIDS(I)%ISMCBP(1:NT)
!/SMC              CALL W3SMCELL( I, NT, IBPTS, XLon, YLat )
!/SMC              DO IX = 1, NT
!  Global processor IPBPT and local JSEA, for ISEA spectrum in I grid.
!/SMC                 ISEA = IBPTS(IX)
!/SMC                 JSEA = 1 + (ISEA - 1)/NP
!/SMC                 IPBPT(IX) = ICROOT - 1 + ISEA - (JSEA - 1)*NP
!/SMC                 JBPTS(IX) = JSEA 
!/SMC              ENDDO
!
            ENDIF     !!  RLGTYPE
...
!/MPI          CALL MPI_BARRIER (MPI_COMM_MWAVE,IEER)
!
! Then broadcast IBPTS, IPBPT, XLon, and YLat to all PEs
!/MPI          CALL MPI_BCAST( IBPTS(1), NT, MPI_INTEGER,  &
!/MPI                          ICROOT-1, MPI_COMM_MWAVE, IEER) 
!/MPI          CALL MPI_BCAST( JBPTS(1), NT, MPI_INTEGER,  &
!/MPI                          ICROOT-1, MPI_COMM_MWAVE, IEER) 
!/MPI          CALL MPI_BCAST( IPBPT(1), NT, MPI_INTEGER,  &
!/MPI                          ICROOT-1, MPI_COMM_MWAVE, IEER) 
!/MPI          CALL MPI_BCAST( XLon(1), NT, MPI_REAL,  &
!/MPI                          ICROOT-1, MPI_COMM_MWAVE, IEER) 
!/MPI          CALL MPI_BCAST( YLat(1), NT, MPI_REAL,  &
!/MPI                          ICROOT-1, MPI_COMM_MWAVE, IEER) 
...
!/MPI/!/SMC          IF( IMPROC .EQ. JCROOT ) THEN
!/SMC              CALL W3SMCGMP( J, NT, XLon, YLat, STORES(I,J)%MSDBP )
!/MPI/!/SMC          ENDIF
!
! Then broadcast the results to all PEs
!/MPI/!/SMC          CALL MPI_BCAST( STORES(I,J)%MSDBP(1), NT, MPI_INTEGER,  &
!/MPI/!/SMC                          JCROOT-1, MPI_COMM_MWAVE, IEER) 
!
!  Need to wait for all PEs get these values.
!/MPI/!/SMC          CALL MPI_BARRIER( MPI_COMM_MWAVE, IEER)
!
!/SMC              STORES(I,J)%ICVBP = IBPTS 
!/SMC              STORES(I,J)%JCVBP = JBPTS 
!/SMC              STORES(I,J)%IPCVB = IPBPT 
!
...
!
              ...
              STORES(I,J)%NFIN = NTL 
!
!/MPI/!/SMC          IF( IMPROC .EQ. NMPERR )              &
!/SMC              WRITE(MDSE,1060) I, NT, J, NTL 
!
...
!
!/MPI/!/SMC        IF( IMPROC .EQ. NMPERR )              &
!/SMC        WRITE(MDSE,*) " *** WMSMCEQL completed from PE ", IMPROC
...
!
!/T 9010 FORMAT ( ' TEST WMSMCEQL : STARTING LOOP OVER GRIDS')
!/T 9011 FORMAT ( ' TEST WMSMCEQL : I, RANK :',2I4)
!/T 9012 FORMAT ( '               GRID ',I3,' HAS SAME RANK')
!/T 9013 FORMAT ( '               ',A)
!
!/T 9020 FORMAT ( ' TEST WMSMCEQL : GENERATING DISTANCE MAP GRID ',I3)
!/T 9024 FORMAT ( ' TEST WMSMCEQL : FINAL MAP FOR X RANGE ',2I6)
!/T 9025 FORMAT (2X,65I2)
!
!/T 9030 FORMAT ( ' TEST WMSMCEQL : DEPENDENCE INFORMATION GRID ',I3)
!/T 9031 FORMAT ( '               CHECKING GRID ',I3)
!/T 9032 FORMAT ( '               POINTS USED/AVAIL :',2I6)
!/T 9033 FORMAT ( '               TOTAL/GRIDS/OUT   :',3I6)
!/T 9034 FORMAT ( '               LOCAL PER GRID    :',15I6)
!/T 9035 FORMAT ( '               SENDING PER GRID  :',15I6)
!/T 9036 FORMAT ( ' TEST WMSMCEQL : NUMBER OF CONTRIBUTING GRIDS MAP')
!/T 9037 FORMAT (2X,65I2)
!
!/T 9040 FORMAT ( ' TEST WMSMCEQL : GRID ',I2,'-',I2,' CLEAR STORAGE')
!/T 9041 FORMAT ( ' TEST WMSMCEQL : GRID ',I2,'-',I2,' STORAGE SIZE',I6)
!/T 9042 FORMAT ( '               RECV ',I2,'-',I2,' CLEAR STORAGE')
!/T 9043 FORMAT ( '               RECV ',I2,'-',I2,' STORAGE SIZE',2I6)
!/T 9044 FORMAT ( '               SEND ',I2,'-',I2,' CLEAR STORAGE')
!/T 9045 FORMAT ( '               SEND ',I2,'-',I2,' STORAGE SIZE',I6)
!
!/T 9050 FORMAT ( ' TEST WMSMCEQL : GRDEQL DIMENSIONED AT ',I2)
!/T 9051 FORMAT ( ' TEST WMSMCEQL : GRDEQL :')
!/T 9052 FORMAT ( '                 ',2i4,' : ',20I3)
!
!/
!/ End of WMSMCEQL  -------------------------------------------------- /
