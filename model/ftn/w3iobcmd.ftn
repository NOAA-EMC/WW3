...
!
!/SMC      USE W3PSMCMD, ONLY: W3SMCGMP
!
      IMPLICIT NONE
!/
!/ ------------------------------------------------------------------- /
...
!/SMC !Li   For SMC grid check whether boundary points are within cell area.
!/SMC          ELSE IF( GTYPE .EQ. SMCTYPE ) THEN
!/SMC            CALL W3SMCGMP( IGRD, NBI, XBPI, YBPI, ISBPI )
!/SMC            IF ( IAPROC .EQ. NAPERR ) WRITE (NDSE,920)    &
!/SMC               ( ISBPI(I), XBPI(I), YBPI(I), I=1,NBI ) 
          ELSE 
            DO I=1, NBI
