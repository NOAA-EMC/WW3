...
!/SMC      USE W3SERVMD, ONLY: W3LLTOEQ 
...
!/SMC      INTEGER      :: NGUI, NGVJ,  NAUI, NAVJ 
!
...
!/SMC      REAL                    :: PoLonAC, PoLatAC
!/SMC      INTEGER, ALLOCATABLE    :: IJKCelAC(:,:),IJKUFcAC(:,:),IJKVFcAC(:,:)
!/SMC      REAL,    ALLOCATABLE    :: XLONAC(:),YLATAC(:),ELONAC(:),ELATAC(:) 
!
...
!/NL5      REAL                    :: NL5DPT, NL5OML
!/NL5      INTEGER                 :: NL5DIS, NL5KEV, NL5IPL, NL5PMX
!/DB1      REAL                    :: BJALFA, BJGAM
!/DB1      LOGICAL                 :: BJFLAG
!/PR2      REAL                    :: DTIME
!
!/SMC      REAL                    :: DTIMS, CFLSM, RFMAXD, SYMR, YJ0R
!/SMC      LOGICAL                 :: UNO3, AVERG, SEAWND, Arctic 
!/SMC      CHARACTER               :: PNSMC*30 
!
...
!/NL5      NAMELIST /SNL5/ NL5DPT, NL5OML, NL5DIS, NL5KEV, NL5IPL, NL5PMX
...
!/SMC      NAMELIST /PSMC/ CFLSM, DTIMS, RFMAXD, Arctic, AVERG, UNO3, &
...
!/NL4      IALT = ALTLP
!
!/NL5      CALL READNL ( NDSS, 'SNL5', STATUS )
!/NL5      WRITE (NDSO,922) STATUS
!/NL5      NL5DPT = MAX(0., MIN(NL5DPT, 3000.))
!/NL5      NL5DIS = MAX(0 , MIN(NL5DIS, 1))
!/NL5      NL5KEV = MAX(0 , MIN(NL5KEV, 1))
!/NL5      NL5IPL = MAX(0 , MIN(NL5IPL, 1))
!/NL5      IF (NL5DIS .EQ. 1) NL5IPL = 0
!/NL5      IF (NL5PMX .GT. 0) NL5PMX = MAX(10, NL5PMX)
!/NL5      WRITE (NDSO,923) NL5DPT, NL5OML, NL5DIS, NL5KEV, NL5IPL, NL5PMX
!/NL5      QR5DPT = NL5DPT
!/NL5      QR5OML = NL5OML
!/NL5      QI5DIS = NL5DIS
!/NL5      QI5KEV = NL5KEV
...
!/PR2      LATMIN = 70.
!
!/SMC !!   Default values of SMC grid parameters.  JGLi06Apr2021
!/SMC      NCel   =  1
!/SMC      NUFc   =  1
!/SMC      NVFc   =  1
!/SMC      NGLO   =  1
...
!/SMC      CFLSM  = MAX ( 0. , CFLSM )
!/SMC      DTIMS  = MAX ( 0. , DTIMS )
!/SMC      RFMAXD = MIN ( 80.0, ABS(RFMAXD) )
!/SMC      Refran = RFMAXD * DERA 
!/SMC  !! Printing out SMC grid parameters. 
!/SMC      WRITE (NDSO,1950)
!/SMC      WRITE (NDSO,1951) PNSMC
!/SMC      WRITE (NDSO,1953) CFLSM, DTIMS/3600., RFMAXD
!
!/SMC      FUNO3  = UNO3
!/SMC      FVERG  = AVERG
!/SMC      FSWND  = SEAWND
...
!/NL5          WRITE (NDSO,2922) QR5DPT, QR5OML, QI5DIS, QI5KEV, QI5IPL, QI5PMX
...
!/SMC          WRITE (NDSO,2954) CFLSM, DTIMS, Arctic, RFMAXD, UNO3, &
...
!/SMC
!/SMC !! Overwrite 2 parameters for SMC grid.  JGLi03Mar2021
!/SMC       DTMS   = DTIMS 
!/SMC       CTMAX  = CFLSM
!
!/SMC       IF (FLGNML) THEN
...
!/SMC
!
!/SMC !! 7.j  Read Arctic grid cell and boundary cell integer arrays.  
!/SMC       IF( ARCTC ) THEN    
!/SMC
!/SMC       IF (FLGNML) THEN
!/SMC         NDSTR = NML_SMC%MBARC%IDF
!/SMC         IDLA = NML_SMC%MBARC%IDLA
!/SMC         IDFM = NML_SMC%MBARC%IDFM
!/SMC         RFORM = TRIM(NML_SMC%MBARC%FORMAT)
!/SMC         TNAME = TRIM(NML_SMC%MBARC%FILENAME)
!/SMC       ELSE  
!/SMC         CALL NEXTLN ( COMSTR , NDSI , NDSE )
!/SMC         READ (NDSI,*,END=2001,ERR=2002) NDSTR, IDLA, IDFM, RFORM, TNAME
!/SMC       END IF
!/SMC       OPEN (NDSTR,FILE=TRIM(FNMPRE)//TNAME,             &
!/SMC                   FORM='FORMATTED',STATUS='OLD',ERR=2000)
!/SMC       READ (NDSTR,*) NARC, NBGL, NBAC
!/SMC       WRITE (NDSO,4015)  NARC, NBGL, NBAC
!/SMC
!/SMC       ALLOCATE (   IJKCelAC( 5, NARC) )
!/SMC       CALL INA2I ( IJKCelAC, 5, NARC, 1, 5, 1, NARC, NDSTR, NDST, NDSE, &
!/SMC                    IDFM, RFORM, IDLA, 1, 0)
!/SMC       CLOSE(NDSTR)
!/SMC !!Li     Offset to change Equator index = 0 to regular grid index JEQT
!/SMC       IJKCelAC( 2, :) = IJKCelAC( 2, :) + JEQT 
!/SMC       IJKCelAC( 1, :) = IJKCelAC( 1, :) + ISHFT
!/SMC
!/SMC       WRITE (NDSO,4016) TNAME
!/SMC       WRITE (NDSO,4006)    1,(IJKCelAC(ix,    1), ix=1,5)
!/SMC       WRITE (NDSO,4006) NARC,(IJKCelAC(ix, NARC), ix=1,5)
!/SMC       WRITE (NDSO,*) ' '
!/SMC
!/SMC       IF (FLGNML) THEN
!/SMC         NDSTR = NML_SMC%AISID%IDF
!/SMC         IDLA = NML_SMC%AISID%IDLA
!/SMC         IDFM = NML_SMC%AISID%IDFM
!/SMC         RFORM = TRIM(NML_SMC%AISID%FORMAT)
!/SMC         TNAME = TRIM(NML_SMC%AISID%FILENAME)
!/SMC       ELSE  
!/SMC         CALL NEXTLN ( COMSTR , NDSI , NDSE )
!/SMC         READ (NDSI,*,END=2001,ERR=2002) NDSTR, IDLA, IDFM, RFORM, TNAME
!/SMC       END IF
...
!/SMC                   FORM='FORMATTED',STATUS='OLD',ERR=2000)
!/SMC       READ (NDSTR,*)  NAUI
!/SMC       WRITE (NDSO,4017)   NAUI
!/SMC
!/SMC       ALLOCATE (   IJKUFcAC( 7, NAUI) )
!/SMC       CALL INA2I ( IJKUFcAC, 7, NAUI, 1, 7, 1, NAUI, NDSTR, NDST, NDSE, &
!/SMC                    IDFM, RFORM, IDLA, 1, 0)
!/SMC       CLOSE(NDSTR)
!/SMC !!Li     Offset to change Equator index = 0 to regular grid index
!/SMC       IJKUFcAC( 2, :) = IJKUFcAC( 2, :) + JEQT 
!/SMC       IJKUFcAC( 1, :) = IJKUFcAC( 1, :) + ISHFT
!/SMC !!Li     Offset Arctic cell sequential numbers by global cell number NGLO
!/SMC       DO  IP=1, NAUI
!/SMC         DO  IX=4,7
!/SMC       IF( IJKUFcAC(IX,IP) > 0 ) IJKUFcAC(IX,IP) = IJKUFcAC(IX,IP) + NGLO
!/SMC         ENDDO
!/SMC       ENDDO
!/SMC
!/SMC       WRITE (NDSO,4018) TNAME
!/SMC       WRITE (NDSO,4009)    1,(IJKUFcAC(ix,    1), ix=1,7)
!/SMC       WRITE (NDSO,4009) NAUI,(IJKUFcAC(ix, NAUI), ix=1,7)
!/SMC       WRITE (NDSO,*) ' '
!/SMC
!/SMC       IF (FLGNML) THEN
!/SMC         NDSTR = NML_SMC%AJSID%IDF
!/SMC         IDLA = NML_SMC%AJSID%IDLA
!/SMC         IDFM = NML_SMC%AJSID%IDFM
!/SMC         RFORM = TRIM(NML_SMC%AJSID%FORMAT)
!/SMC         TNAME = TRIM(NML_SMC%AJSID%FILENAME)
!/SMC       ELSE  
!/SMC         CALL NEXTLN ( COMSTR , NDSI , NDSE )
!/SMC         READ (NDSI,*,END=2001,ERR=2002) NDSTR, IDLA, IDFM, RFORM, TNAME
!/SMC       END IF
...
!/SMC                   FORM='FORMATTED',STATUS='OLD',ERR=2000)
!/SMC       READ (NDSTR,*) NAVJ 
!/SMC       WRITE (NDSO,4019)   NAVJ
!/SMC
!/SMC       ALLOCATE (   IJKVFcAC( 8, NAVJ) )
!/SMC       CALL INA2I ( IJKVFcAC, 8, NAVJ, 1, 8, 1, NAVJ, NDSTR, NDST, NDSE, &
!/SMC                    IDFM, RFORM, IDLA, 1, 0)
!/SMC       CLOSE(NDSTR)
!/SMC !!Li     Offset to change Equator index = 0 to regular grid index
!/SMC       IJKVFcAC( 2, :) = IJKVFcAC( 2, :) + JEQT 
!/SMC       IJKVFcAC( 1, :) = IJKVFcAC( 1, :) + ISHFT
!/SMC !!Li     Offset Arctic cell sequential numbers by global cell number NGLO
!/SMC       DO  IP=1, NAVJ
!/SMC         DO  IY=4,7
!/SMC       IF( IJKVFcAC(IY,IP) > 0 ) IJKVFcAC(IY,IP) = IJKVFcAC(IY,IP) + NGLO
!/SMC         ENDDO
!/SMC       ENDDO
!/SMC
!/SMC       WRITE (NDSO,4020) TNAME
!/SMC       WRITE (NDSO,4012)    1,(IJKVFcAC(ix,    1), ix=1,8)
!/SMC       WRITE (NDSO,4012) NAVJ,(IJKVFcAC(ix, NAVJ), ix=1,8)
!/SMC       WRITE (NDSO,*) ' '
!/SMC
!/SMC !!Li  Reset total cell and face numbers
!/SMC       NCel = NGLO + NARC
!/SMC       NUFc = NGUI + NAUI
!/SMC       NVFc = NGVJ + NAVJ
!/SMC !!Li  Also append Arctic part into base level sub-loops
!/SMC       NLvCelsk(NRLv)=NLvCelsk(NRLv)+NARC 
!/SMC       NLvUFcsk(NRLv)=NLvUFcsk(NRLv)+NAUI 
!/SMC       NLvVFcsk(NRLv)=NLvVFcsk(NRLv)+NAVJ 
!/SMC !!Li  Reset NBAC to total number of boundary cells.
!/SMC       NBAC = NBGL + NBAC
!/SMC
!/SMC       ENDIF  !! ARCTC section.
!/SMC
!/SMC       ENDIF  !! GTYPE .EQ. SMCTYPE 
!
!--- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! 8.  Finalize status maps
...
!
        ...
        END IF   ! GTYPE
!      
!/SMC !!Li  Shelter MAPSTA LLG definition for SMC 
!/SMC      IF( GTYPE .NE. SMCTYPE ) THEN
!
      MAPSTA = TMPSTA
      MAPFS  = 0
!
...
!/DEBUGSTP        FLUSH(740)
!
!/SMC !!Li SMC grid definition of mapping arrays.
!/SMC      ELSE 
!
...
!/SMC       IJKCel(:, 1:NGLO)=IJKCelin(:, 1:NGLO)
!/SMC       IJKUFc(:, 1:NGUI)=IJKUFcin(:, 1:NGUI)
!/SMC       IJKVFc(:, 1:NGVJ)=IJKVFcin(:, 1:NGVJ)
!/SMC !Li     Append Arctic part
!/SMC       IF( ARCTC ) THEN
!/SMC       IJKCel(:, NGLO+1:NCel)=IJKCelAC(:, 1:NARC)
!/SMC       IJKUFc(:, NGUI+1:NUFc)=IJKUFcAC(:, 1:NAUI)
!/SMC       IJKVFc(:, NGVJ+1:NVFc)=IJKVFcAC(:, 1:NAVJ)
!/SMC       ENDIF !! ARCTC
!/SMC
!/SMC       WRITE (NDSO,4026) 
...
!/SMC
!/SMC       DO ISEA=1, NCel
!/SMC !Li   There is no polar cell row so it is mapped to last row.
!/SMC        IF( ARCTC .AND. (ISEA .EQ. NCel) ) THEN 
!/SMC             IX=1
!/SMC             IY=NY
!/SMC             IK=1
!/SMC             JS=1
...
!/SMC !Li   Arctic polar cell does not need COS(LAT), set 1 row down.
!/SMC          IF(Y .GE. HPI-0.1*SYMR) Y=HPI - SYMR*0.5*FLOAT( MRFct )
!/SMC 
...
!/SMC       ENDDO
!/SMC       IF(NBISMC .GT. 0) THEN
!/SMC !Li   Save input boundary SMC list to ISMCBP(NBSMC)
!/SMC          ISMCBP(1:NBISMC) = NBICelin(1:NBISMC)
...
!/SMC 
!
!/SMC !Li   Define rotation angle for Arctic cells.
!/SMC       IF( ARCTC ) THEN
!/SMC 
!/SMC       PoLonAC = 179.999
!/SMC       PoLatAC =   0.001
!/SMC       ALLOCATE( XLONAC(NARC),YLATAC(NARC),ELONAC(NARC),ELATAC(NARC) )
!/SMC       DO ISEA=NGLO+1, NCel
!/SMC !Li   There is no polar cell row so it is mapped to last row.
!/SMC          IF(ISEA .EQ. NCel) THEN
!/SMC             IX=1
!/SMC             IY=NY
!/SMC             IK=1
!/SMC             JS=1
...
!/SMC             JS=MAX(1, IJKCel(4,ISEA)/MRFct)
!/SMC          ENDIF
!/SMC          XLONAC(ISEA-NGLO)= X0 + REAL(IX-1+IK/2)*SX
!/SMC          YLATAC(ISEA-NGLO)= Y0 + REAL(IY-1+JS/2)*SY
!/SMC       ENDDO 
!/SMC
!/SMC        CALL W3LLTOEQ ( YLATAC, XLONAC, ELATAC, ELONAC,   &
!/SMC      &                 ANGARC, PoLatAC, PoLonAC, NARC  )
!/SMC
!/SMC       WRITE (NDSO,4037)  NARC 
!/SMC       WRITE (NDSO,4038) (ANGARC(ix), ix=1,NARC,NARC/8)
!/SMC
!
!/SMC !Li   Mapping Arctic boundary cells with inner model cells
!/SMC       DO IP=1, NBAC 
!/SMC             IX=IJKCel(1,IP+NGLO)
!/SMC             IY=IJKCel(2,IP+NGLO)
!/SMC          DO ISEA=1, NGLO 
!/SMC             IF( (IX .EQ. IJKCel(1,ISEA)) .AND.      &
!/SMC      &          (IY .EQ. IJKCel(2,ISEA)) ) THEN 
!/SMC                 ICLBAC(IP) = ISEA
!/SMC             ENDIF
!/SMC          ENDDO
!/SMC       ENDDO
!/SMC       WRITE (NDSO,4039)  NBAC 
!/SMC       WRITE (NDSO,4040) (ICLBAC(ix), ix=1,NBAC,NBAC/8)
!/SMC
!/SMC !Li   Redefine GCT term factor for Arctic part or the netative of 
!/SMC !Li   tangient of rotated latitude divided by radius. JGLi14Sep2015
!/SMC       DO ISEA=NGLO+1, NCel-1
!/SMC          CTHG0S(ISEA)= - TAN( ELATAC(ISEA-NGLO)*DERA ) / RADIUS
!/SMC       ENDDO 
!/SMC          CTHG0S(NCel)=0.0
!/SMC       
!/SMC       ENDIF  !! ARCTC section.
!
!/SMC      ENDIF  !! (GTYPE .NE. SMCTYPE) ELSE SMCTYPE block. 
!
...
!
!/NL5  922 FORMAT(/'  Nonlinear interactions (GKE) ',A/              &
!/NL5        ' --------------------------------------------------')
!/NL5  923 FORMAT ( '   Constant water depth (in meter) : ', F7.1/ &
!/NL5               '   Quasi-resonant quartets cut-off : ', F8.2/ &
!/NL5               '   Discretiz. of GKE (0:Con., 1:GS): ', I5/   &
!/NL5               '   GKE (0: GS13-JFM, 1: J03-JPO)   : ', I5/   &
!/NL5               '   Interp (0: nearest, 1: bilinear): ', I5/   &
!/NL5               '   Mixing (0: no, N: N Tm, -1: b_T): ', I5/)
!/NL5 2922 FORMAT ( '  &SNL5 NL5DPT =', F7.1, ', NL5OML =', F5.2, &
!/NL5               ', NL5DIS =', I2.1, ', NL5KEV =', I2.1,       &
!/NL5               ', NL5IPL =', I2.1, ', NL5PMX =', I5.1, ' /')
!
...
!/PR2                        ', LATMIN =',F5.1,' /')
!
!/SMC 1950 FORMAT (/'  SMC grid parameters : '/                            &
!/SMC               ' --------------------------------------------------')
!/SMC 1951 FORMAT ( '       Type of scheme (structured) :',1X,A)
!/SMC 1953 FORMAT ( '       Max propagation CFL number  :',F9.3/      &
!/SMC               '       Effective swell age     (h) :',F8.2/      &
!/SMC               '       Maximum refraction  (degr.) :',F8.2/)
!/SMC 2954 FORMAT ( '  &PSMC CFLSM  =',F5.2,', DTIMS  =', F9.1/     &
!/SMC               '        Arctic =',L5,  ', RFMAXD =', F9.2/     & 
...
!/SMC 4014 FORMAT (2I8)
!/SMC 4015 FORMAT ( '       ARC NARC   = ',6I9)
!/SMC 4016 FORMAT ( '       IJKCel(5,NARC) read from ', A)
!/SMC 4017 FORMAT ( '       ARC NAUI   = ',6I9)
!/SMC 4018 FORMAT ( '       IJKUFc(7,NAUI) read from ', A)
!/SMC 4019 FORMAT ( '       ARC NAVJ   = ',6I9)
!/SMC 4020 FORMAT ( '       IJKVFc(8,NAVJ) read from ', A)
...
!/SMC 4037 FORMAT ( '       Arctic AngArc defined as ',I6)
!/SMC 4038 FORMAT (9F8.2)
!/SMC 4039 FORMAT ( '       Arctic ICLBAC defined as ',I6)
!/SMC 4040 FORMAT (9I8)
...
!/NL5                        CASE('SNL5')
!/NL5                          READ (NDS,NML=SNL5,END=801,ERR=802,IOSTAT=J)
!/NLS                        CASE('SNLS')
