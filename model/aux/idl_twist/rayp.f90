...
! Starts first chunk
      ...
      DO nstep=1,100000
         ...
         IF (yrad(1).lt.20.or.yrad(1).gt.nx-20) IFlag=0
         IF (yrad(2).lt.20.or.yrad(2).gt.ny-20) IFlag=0
         IF (dl.gt.depthmax) IFlag=1
         IF (dl.lt.depthmin) IFlag=0
         IF (iflag.eq.0.or.iflag.eq.1) THEN
            !WRITE(6,*) dl,depthmin,nchunk*timestep+tchunk
            nchunk=nchunk+1
            xch(nchunk)=yrad(1)*sx/1000.
            ych(nchunk)=yrad(2)*sy/1000.
            ach(nchunk)=yrad(3)*180/pi
...
!**********************************************************************
   ...
   REAL xg,yg,cg,depth,dcdx,dcdy,ca,sa  
   ...
   ca=cos(y(3))
   sa=sin(y(3))
   dydt(1)=cg*ca/sx        ! x and y are the coordinates in grid units
   dydt(2)=cg*sa/sy        
...
!**********************************************************************
SUBROUTINE speed(xg,yg,cg,depth,dcdx,dcdy)
!**********************************************************************
...
!**********************************************************************
   ...
   REAL xp,yp,sig,aka,yhat,t,a,dddx,dddy,dcdd
...
!                 Get the grid itself
   ALLOCATE(grid(nx,ny))
      ...
      !WRITE(6,*) 'speed',xs,ys,c,dcdx,dcdy
      a_step=dtheta
