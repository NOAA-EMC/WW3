...
!/SMC        REAL, POINTER         :: CTRNX(:), CTRNY(:), CLATF(:)
        ...
        LOGICAL          :: GUGINIT
!/REF1        REAL, POINTER    :: REFLC(:,:)  ! reflection coefficient
!/REF1        INTEGER, POINTER :: REFLD(:,:)  ! reflection direction
...
!/PDLIB        INTEGER               :: NBND_MAP
!/PDLIB        INTEGER, POINTER      :: INDEX_MAP(:)
!/PDLIB        INTEGER, POINTER      :: MAPSTA_LOC(:)
...
!/UOST         CHARACTER(LEN=256)      :: UOSTFILELOCAL, UOSTFILESHADOW
!/UOST         LOGICAL, ALLOCATABLE    :: UOST_LCL_OBSTRUCTED(:,:), UOST_SHD_OBSTRUCTED(:,:)
!/UOST         INTEGER*1, ALLOCATABLE  :: UOSTLOCALALPHA(:,:,:,:), UOSTLOCALBETA(:,:,:,:)
!/UOST         INTEGER*1, ALLOCATABLE  :: UOSTSHADOWALPHA(:,:,:,:), UOSTSHADOWBETA(:,:,:,:)
!/UOST         REAL*4, ALLOCATABLE     :: UOSTCELLSIZE(:,:,:)
!/UOST         REAL                    :: UOSTABMULTFACTOR = 100
!/UOST         REAL                    :: UOSTCELLSIZEFACTOR = 1000
!/UOST         REAL                    :: UOSTLOCALFACTOR = 1
!/UOST         REAL                    :: UOSTSHADOWFACTOR = 1
!/UOST         LOGICAL                 :: UOSTENABLED = .true.
...
!/PR3        REAL                  :: WDCG, WDTH
!/SMC        REAL                  :: DTMS, Refran
!/SMC        LOGICAL               :: FUNO3, FVERG, FSWND, ARCTC  
      END TYPE PROP
!
      TYPE FLDP
         REAL :: DUMMY
...
!/NL5        REAL                  :: QR5DPT, QR5OML
!/NL5        INTEGER               :: QI5DIS, QI5KEV, QI5IPL, QI5PMX
!/NL5        INTEGER(KIND=8)       :: QI5NNZ
...
!/UOST      TYPE UOSTP
!/UOST        CHARACTER(LEN=256)    :: UOSTFILELOCAL, UOSTFILESHADOW
!/UOST        REAL              :: UOSTFACTORLOCAL, UOSTFACTORSHADOW
!/UOST      END TYPE UOSTP
...
!/REF1      REAL,    POINTER        :: REFLC(:,:)
!/REF1      INTEGER, POINTER        :: REFLD(:,:)
...
!/SMC        INTEGER, POINTER :: NGLO, NARC, NBGL, NBAC, NBSMC
!/SMC        INTEGER, POINTER :: NLvCel(:), NLvUFc(:), NLvVFc(:)
!/SMC        INTEGER, POINTER :: IJKCel(:,:), IJKUFc(:,:), IJKVFc(:,:)
!/SMC        INTEGER, POINTER :: ISMCBP(:),   ICLBAC(:)
!
...
!/SMC      REAL, POINTER           :: DTMS, Refran
!/SMC      LOGICAL, POINTER        :: FUNO3, FVERG, FSWND, ARCTC  
!/
...
!/ST3                                 SSDSC1, SSDSC2, SSDSC3, SSDSBR,      &
!/ST3                                 SSDSP, WWNMEANP, SSTXFTF, SSTXFTWN,  &
!/ST3                                 SSDSC4, SSDSC5, SSDSC6, SSDSBT,      &
!/ST3                                 DDELTA1, DDELTA2,                    &
...
!/NL5      REAL, POINTER           :: QR5DPT, QR5OML
!/NL5      INTEGER, POINTER        :: QI5DIS, QI5KEV, QI5IPL, QI5PMX
!/NL5      INTEGER(KIND=8), POINTER:: QI5NNZ
...
!/
!/UOST!/ Data aliases for structure UOSTP(S)
!/UOST      CHARACTER(LEN=:), POINTER   :: UOSTFILELOCAL, UOSTFILESHADOW
!/UOST      REAL, POINTER           :: UOSTFACTORLOCAL, UOSTFACTORSHADOW
!/
...
!/SMC       INTEGER, INTENT(IN)     :: MCel, MUFc, MVFc, MRLv, MBSMC 
!/SMC       INTEGER, INTENT(IN)     :: MARC, MBAC, MSPEC 
!/
!/ ------------------------------------------------------------------- /
!/ Local parameters
!/
!/SMC       INTEGER                 :: IARC, IBAC, IBSMC
!/S      INTEGER, SAVE           :: IENT = 0
!/
...
!
!/SMC !! Arctic part related variables, declare minimum 1 element.
!/SMC      IARC = MARC
!/SMC      IF( MARC .LE. 1 ) IARC = 1
!/SMC      IBAC = MBAC
!/SMC      IF( MBAC .LE. 1 ) IBAC = 1
!/SMC      IBSMC = MBSMC
!/SMC      IF( MBSMC .LE. 1 ) IBSMC = 1
!/SMC      ALLOCATE ( GRIDS(IMOD)%ICLBAC(IBAC),       &
!/SMC                 GRIDS(IMOD)%ANGARC(IARC),       &
!/SMC                 GRIDS(IMOD)%SPCBAC(MSPEC,IBAC), &
!/SMC                 GRIDS(IMOD)%ISMCBP(IBSMC),      &
!/SMC                 STAT=ISTAT                      )
!/SMC      CHECK_ALLOC_STATUS ( ISTAT )
!
!/SMC !! All SMC grid related varialbes are initialised in case SMC  
!/SMC !! switch is selected but SMCTYPE is not used.  JGLi08Mar2021 
!/SMC      GRIDS(IMOD)%NLvCel(:) = 0 
!/SMC      GRIDS(IMOD)%NLvUFc(:) = 0
!/SMC      GRIDS(IMOD)%NLvVFc(:) = 0 
!/SMC      GRIDS(IMOD)%ISMCBP(:) = 0
!/SMC      GRIDS(IMOD)%ICLBAC(:) = 0
!/SMC      GRIDS(IMOD)%IJKCel(:,:) = 0 
!/SMC      GRIDS(IMOD)%IJKUFc(:,:) = 0 
!/SMC      GRIDS(IMOD)%IJKVFc(:,:) = 0 
!/SMC      GRIDS(IMOD)%CTRNX(:)  = 0.0 
!/SMC      GRIDS(IMOD)%CTRNY(:)  = 0.0
!/SMC      GRIDS(IMOD)%CLATF(:)  = 0.0 
!/SMC      GRIDS(IMOD)%ANGARC(:) = 0.0
!
...
!
!/REF1      GRIDS(IMOD)%RREF(:)=.FALSE.
!/REF1      GRIDS(IMOD)%REFPARS(:)=0.
!
...
!/IC2       ALLOCATE ( GRIDS(IMOD)%IC2PARS(9), STAT=ISTAT )
...
!/IC4       CHECK_ALLOC_STATUS ( ISTAT )
!/IC5       ALLOCATE ( GRIDS(IMOD)%IC5PARS(9), STAT=ISTAT )
!/IC5       CHECK_ALLOC_STATUS ( ISTAT )
!/IS2       ALLOCATE ( GRIDS(IMOD)%IS2PARS(24), STAT=ISTAT )
...
!
!/SMC      NGLO   => GRIDS(IMOD)%NGLO 
!/SMC      NARC   => GRIDS(IMOD)%NARC 
!/SMC      NBGL   => GRIDS(IMOD)%NBGL 
!/SMC      NBAC   => GRIDS(IMOD)%NBAC 
!/SMC      NBSMC  => GRIDS(IMOD)%NBSMC
!
...
!/REF1      REFLC  => GRIDS(IMOD)%REFLC
!/REF1      REFLD  => GRIDS(IMOD)%REFLD
...
!/IC5      IC5PARS => GRIDS(IMOD)%IC5PARS
...
!/RTD      FLAGUNR  => GRIDS(IMOD)%FLAGUNR
...
!/PDLIB      NBND_MAP => GRIDS(IMOD)%NBND_MAP
!/PDLIB      INDEX_MAP => GRIDS(IMOD)%INDEX_MAP
!/PDLIB      MAPSTA_LOC => GRIDS(IMOD)%MAPSTA_LOC
...
!/PDLIB      IOBP_loc     => GRIDS(IMOD)%IOBP_loc
!/PDLIB      IOBPD_loc     => GRIDS(IMOD)%IOBPD_loc
...
!/SMC          ISMCBP => GRIDS(IMOD)%ISMCBP
!/SMC          CTRNX  => GRIDS(IMOD)%CTRNX
!/SMC          CTRNY  => GRIDS(IMOD)%CTRNY
!/SMC          CLATF  => GRIDS(IMOD)%CLATF
!
!/SMC          ICLBAC => GRIDS(IMOD)%ICLBAC
!/SMC          ANGARC => GRIDS(IMOD)%ANGARC
!/SMC          SPCBAC => GRIDS(IMOD)%SPCBAC
!
          GSU  => GRIDS(IMOD)%GSU
!
        END IF
...
!/SMC      DTMS   => MPARS(IMOD)%PROPS%DTMS
...
!/SMC      ARCTC  => MPARS(IMOD)%PROPS%ARCTC
!
!     Structure FLDP
!
...
!/ST4      SSDSBT   => MPARS(IMOD)%SRCPS%SSDSBT
...
!/ST6      SIN6WS => MPARS(IMOD)%SRCPS%SIN6WS
!/ST6      SIN6FC => MPARS(IMOD)%SRCPS%SIN6FC
...
!/NL5      QR5DPT => MPARS(IMOD)%SNLPS%QR5DPT
!/NL5      QR5OML => MPARS(IMOD)%SNLPS%QR5OML
!/NL5      QI5DIS => MPARS(IMOD)%SNLPS%QI5DIS
!/NL5      QI5KEV => MPARS(IMOD)%SNLPS%QI5KEV
!/NL5      QI5NNZ => MPARS(IMOD)%SNLPS%QI5NNZ
!/NL5      QI5IPL => MPARS(IMOD)%SNLPS%QI5IPL
!/NL5      QI5PMX => MPARS(IMOD)%SNLPS%QI5PMX
...
!
!
!/UOST      UOSTFILELOCAL => MPARS(IMOD)%UOSTPS%UOSTFILELOCAL
!/UOST      UOSTFILESHADOW => MPARS(IMOD)%UOSTPS%UOSTFILESHADOW
!/UOST      UOSTFACTORLOCAL => MPARS(IMOD)%UOSTPS%UOSTFACTORLOCAL
!/UOST      UOSTFACTORSHADOW => MPARS(IMOD)%UOSTPS%UOSTFACTORSHADOW
!
!     Structure SICPS
!
...
!/MEMCHECK      WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_UG_ALLOCATE SECTION 1'
...
!
      ...
      WRITE (NDST,9003)
#endif
!/MEMCHECK      WRITE(740+IAPROC,*) 'memcheck_____:', 'WW3_UG_ALLOCATE SECTION 2'
!/MEMCHECK      call getMallocInfo(mallinfos)
!/MEMCHECK      call printMallInfo(IAPROC,mallInfos)
