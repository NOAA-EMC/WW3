...
!
#ifdef W3_SMC
      REAL                    :: DTIME, RFMAXD, SYMR, YJ0R
      LOGICAL                 :: UNO3, AVERG, SEAWND 
#endif // W3_SMC
!
#ifdef W3_PR3
      REAL                    :: WDTHCG, WDTHTH
...
#endif // W3_PR2
#ifdef W3_SMC
      NAMELIST /PSMC/ CFLTM, DTIME, LATMIN, RFMAXD, UNO3, AVERG, &
...
!
#ifdef W3_SMC
  !!Li  Switch off regular grid lines by setting the logical
        RGLGRD = .FALSE.
#endif // W3_SMC
!
!--- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
!
#ifdef W3_SMC
 !Li  Default values of smc grid parameters.  JGLi23Nov2015
      LvSMC  =  1
      ISHFT  =  0
      JEQT   =  0
      NBISMC =  0
...
!
      ...
      IF ( DTIME .EQ. 0. ) THEN
          WRITE (NDSO,953) CFLTM, LATMIN, RFMAXD
        ELSE
          WRITE (NDSO,954) CFLTM, DTIME/3600., LATMIN, RFMAXD
        END IF
      DTME   = DTIME
      Refran = RFMAXD * DERA 
      FUNO3  = UNO3
...
#endif // W3_PR2
#ifdef W3_SMC
          WRITE (NDSO,2953) CFLTM, DTIME, LATMIN, RFMAXD, UNO3, &
...
!
 ...
 !        JJ=IJKCel(2,ISEA) - JEQT 
 !        Y = SYMR*( FLOAT(JJ)+0.5*FLOAT(IJKCel(4,ISEA)) )
 ...
 !!Li    Subgrid obstruction is now defined directly from IJKObstr
 !!Li    so old OBSX/Y are no longer used.  JGLi15Oct2014
 !!Li    Transparency is minimum of all merged cells and >= 0.11
 !         TRNMX=1.0
 !         TRNMY=1.0
 !         DO ip = IX, IX+IK-1
 !            TRNMX = MIN( TRNMX, ABS(1.0-OBSX(ip,IY)) )
 !            TRNMY = MIN( TRNMY, ABS(1.0-OBSY(ip,IY)) )
 !         ENDDO
...
!
#ifdef W3_LNX
  820 FORMAT (/'  Experimental linear input.'/)
#endif // W3_LNX
!
#ifdef W3_ST0
...
!
#ifdef W3_STX
  920 FORMAT (/'  Experimental wind input.'/)
#endif // W3_STX
!
#ifdef W3_NL0
...
!
#ifdef W3_NLX
  922 FORMAT (/'  Experimental nonlinear interactions.'/)
#endif // W3_NLX
!
#ifdef W3_NLS
...
!
#ifdef W3_STX
  924 FORMAT (/'  Experimental dissipation.'/)
#endif // W3_STX
!
#ifdef W3_BT0
...
#endif // W3_BT4
#ifdef W3_BTX
  926 FORMAT (/'  Experimental bottom friction.'/)
#endif // W3_BTX
!
#ifdef W3_DB0
...
!
#ifdef W3_DBX
  928 FORMAT (/'  Experimental depth-induced breaking.'/)
#endif // W3_DBX
!
#ifdef W3_TR0
...
#endif // W3_TR0
#ifdef W3_TRX
  930 FORMAT (/'  Experimental triad interactions.'/)
#endif // W3_TRX
!
#ifdef W3_BS0
...
#endif // W3_BS1
#ifdef W3_BSX
  932 FORMAT (/'  Experimental bottom scattering.'/)
#endif // W3_BSX
!
#ifdef W3_XX0
...
#endif // W3_XX0
#ifdef W3_XXX
  934 FORMAT (/'  Experimental unclasified source term.'/)
#endif // W3_XXX
!
#ifdef W3_IC1
...
!
#ifdef W3_SMC
  953 FORMAT ( '       Max propagation CFL number  :',F9.3/      &
               '       Effective swell age     (h) : switched off'/ &
               '       Cut-off latitude    (degr.) :',F8.2/      &
               '       Maximum refraction  (degr.) :',F8.2/)
  954 FORMAT ( '       Max propagation CFL number  :',F9.3/      &
               '       Effective swell age     (h) :',F8.2/      &
               '       Cut-off latitude    (degr.) :',F8.2/      &
               '       Maximum refraction  (degr.) :',F8.2/)
 2953 FORMAT ( '  &PSMC CFLTM  =',F5.2,', DTIME  =', F9.1/     &
               '        LATMIN =',F5.1,', RFMAXD =', F9.2/     & 
               '        UNO3   =',L5,  ', AVERG  =',L5/        &
               '        LvSMC  =',i5,  ', NBISMC =',i9/        &
               '        ISHFT  =',i5,  ', JEQT   =',i9/        &
               '        SEAWND =',L5, ' /')
