...
!       wDEBUG   Log.   I   OPTIONAL logical flag to turn on debug mode.
!                          Default is FALSE.
!     ----------------------------------------------------------------
!
!  4. Subroutines used :
...
!     Single precision pointer interface
      ...
      LOGICAL, INTENT(IN), OPTIONAL :: wDEBUG
...
!/                     POLE, EPS, FNCL, wDEBUG ) RESULT(INGRID)
!/
!/ =================================================================== /
!/
!  1. Purpose :
...
!-----call double precision method
      INGRID = W3GFCL( GSU, XT8, YT8, IS, JS, XS8, YS8, POLE=POLE, &
                       EPS=EPS8, FNCL=FNCL, wDEBUG=wDEBUG )
!
!-----set outputs
      XT = XT8; YT = YT8;
      XS = XS8; YS = YS8;
...
!/ =================================================================== /
!/
!/    FUNCTION W3GFCD_R4( GSU, XT, YT, IS, JS, XS, YS, POLE, EPS, wDEBUG ) &
!/    RESULT(INGRID)
!/
!/ =================================================================== /
!/
...
!/ =================================================================== /
!/
!/    FUNCTION W3GFPT( GSU, XTIN, YTIN, IX, IY, EPS, DCIN, wDEBUG ) &
!/    RESULT(INGRID)
!/
!/ =================================================================== /
!/
...
!/ ------------------------------------------------------------------- /
!/
      FUNCTION W3GFPT_R4( GSU, XTIN, YTIN, IX, IY, EPS, DCIN, wDEBUG ) &
      RESULT(INGRID)
!     Single precision interface
      LOGICAL                 :: INGRID
      TYPE(T_GSU), INTENT(IN) :: GSU
...
!/ ------------------------------------------------------------------- /
!/
      FUNCTION W3GFPT_R8( GSU, XTIN, YTIN, IX, IY, EPS, DCIN, wDEBUG ) &
      RESULT(INGRID)
!     Single precision interface
      LOGICAL                 :: INGRID
      TYPE(T_GSU), INTENT(IN) :: GSU
...
!
      FNCL = LDCIN .GT. ZERO
      INGRID = W3GFCL( GSU, XT, YT, IS, JS, XS, YS, EPS=LEPS, FNCL=FNCL, wDEBUG=LDBG )
...
!-----------non-pole cell: compute relative location using (lon,lat)
            CALL GETPQR(XT,YT,XS,YS,IXR,JXR,EPS=LEPS,wDEBUG=LDBG)
          ELSE
!-----------pole cell: compute relative location using stereographic projection
            ...
            DO I=1,4
              ...
              END DO
            CALL GETPQR(XTC,YTC,XSC,YSC,IXR,JXR,EPS=LEPS,wDEBUG=LDBG)
          ENDIF
          DD = HALF + LDCIN
...
!/ =================================================================== /
!/
!/    FUNCTION W3GFIJ( GSU, XTIN, YTIN, IX, JX, EPS, DCIN, wDEBUG ) &
!/    RESULT(INGRID)
!/
!/ =================================================================== /
!/
...
!/ ------------------------------------------------------------------- /
!/
      FUNCTION W3GFIJ_R4( GSU, XTIN, YTIN, IX, JX, EPS, DCIN, wDEBUG ) &
      RESULT(INGRID)
!     Single precision interface
      LOGICAL                 :: INGRID
      TYPE(T_GSU), INTENT(IN) :: GSU
...
!/ ------------------------------------------------------------------- /
!/
      FUNCTION W3GFIJ_R8( GSU, XTIN, YTIN, IX, JX, EPS, DCIN, wDEBUG ) &
      RESULT(INGRID)
!     Double precision interface
      LOGICAL                 :: INGRID
      TYPE(T_GSU), INTENT(IN) :: GSU
...
!
      FNCL = LDCIN .GT. ZERO
      INGRID = W3GFCL(GSU,XT,YT,IS,JS,XS,YS,POLE=POLE,EPS=LEPS,FNCL=FNCL,wDEBUG=LDBG)
      IF ( .NOT.INGRID .AND. .NOT.FNCL ) RETURN
!
!-----Compute cell relative index space location
      LON0 = SUM(XS)/FOUR; LAT0 = SUM(YS)/FOUR;
...
!/                     DCIN, MASK, MSKC, NNBR, wDEBUG ) RESULT(INGRID)
!/
!/ =================================================================== /
!/
!  1. Purpose :
...
!-----call double precision method
      INGRID = W3GRMP( GSU, XT8, YT8, IS, JS, RW8, &
                       EPS=EPS8, DCIN=DCIN8, &
                       MASK=MASK, MSKC=MSKC, NNBR=NNBR, wDEBUG=wDEBUG )
!
!-----set outputs
      RW = RW8
...
!/                     DCIN, WDTH, MASK, NMSK, wDEBUG ) RESULT(INGRID)
!/
!/ =================================================================== /
!/
!  1. Purpose :
...
!
      ...
      INGRID = W3GRMC( GSU, XT, YT, RTYP, NS, IS, JS, CS8, &
                       ...
                       MASK=MASK, NMSK=NMSK, wDEBUG=wDEBUG )
      IF ( NS.GT.0 ) THEN
          ALLOCATE( CS(NS) )
          CS(:) = CS8(:)
          DEALLOCATE( CS8 )
...
!
      FNCL = LDCIN .GT. ZERO
      INGRID = W3GFCL(GSU,XT,YT,IC,JC,XC,YC,POLE=POLE,EPS=LEPS,FNCL=FNCL,wDEBUG=LDBG)
      IF ( .NOT.INGRID .AND. .NOT.FNCL ) RETURN
!
!-----Compute cell relative index space location
      ...
      IF ( D90-ABS(LAT0).GT.NEAR_POLE ) THEN
!---------non-pole cell: compute relative location using (lon,lat)
          CALL GETPQR(XT,YT,XC,YC,IXR,JXR,EPS=LEPS,wDEBUG=LDBG)
        ELSE
!---------pole cell: compute relative location using stereographic projection
          CALL W3SPLX(LON0,LAT0,ZERO,XT,YT,XTC,YTC)
          DO I=1,4
...
!/ =================================================================== /
!/
!/    FUNCTION W3CKCL( LLG, XT, YT, NS, XS, YS, POLE, EPS, wDEBUG ) &
!/    RESULT(INCELL)
!/
!/ =================================================================== /
!/
...
!/ ------------------------------------------------------------------- /
!/
      FUNCTION W3CKCL_R4( LLG, XT, YT, NS, XS, YS, POLE, EPS, wDEBUG ) &
      RESULT(INCELL)
!     Single precision interface
      LOGICAL                :: INCELL
      LOGICAL, INTENT(IN)    :: LLG
...
!/ ------------------------------------------------------------------- /
!/
      FUNCTION W3CKCL_R8( LLG, XT, YT, NS, XS, YS, POLE, EPS, wDEBUG ) &
      RESULT(INCELL)
!     Double precision interface
      LOGICAL                :: INCELL
      LOGICAL, INTENT(IN)    :: LLG
...
!/
      FUNCTION GSU_CREATE( IJG, LLG, ICLO, LB, UB, XG4, YG4, XG8, YG8, &
                           BBOX_ONLY, NCB, NNP, wDEBUG ) RESULT(GSU)
!     *** INTERNAL SUBROUTINE ***
      TYPE(T_GSU)         :: GSU
      LOGICAL, INTENT(IN) :: IJG
      LOGICAL, INTENT(IN) :: LLG
...
!/ ------------------------------------------------------------------- /
!/
      SUBROUTINE GETPQR( XT, YT, XS, YS, PR, QR, EPS, wDEBUG )
!     *** INTERNAL SUBROUTINE ***
...
!     Local parameters
      INTEGER, PARAMETER :: M = 1 ! order of derivative
      LOGICAL, PARAMETER :: wDEBUG = .FALSE.
      CHARACTER(64) :: FSTR
