...
MODULE W3ADATMD
  ...
  TYPE WADAT
     ...
     REAL, POINTER :: XPRMS(:), XTPMS(:), XPHICE(:)
     REAL, POINTER :: XTAUICE(:,:)
     ...
     REAL, POINTER :: XABA(:), XABD(:), XUBA(:), XUBD(:), XBEDFORMS(:,:), XPHIBBL(:), XTAUBBL(:,:)
     !
     ...
     REAL, POINTER    :: LANGMT(:), LAPROJ(:), LASL(:)   ! W3_CESMCOUPLED
     REAL, POINTER    :: LASLPJ(:), LAMULT(:), ALPHAL(:) ! W3_CESMCOUPLED
     REAL, POINTER    :: ALPHALS(:), USSXH(:), USSYH(:)  ! W3_CESMCOUPLED
#endif
     !
     ! Spatial derivatives
     !
  ...
  !/
#ifdef W3_CESMCOUPLED
  REAL,    POINTER :: LANGMT(:), LAPROJ(:), ALPHAL(:), ALPHALS(:), LAMULT(:), LASL(:)
  ...
  REAL,    POINTER :: THP0(:), FP1(:), THP1(:), HSIG(:)
  ...
  !
  REAL,    POINTER :: DDDX(:,:), DDDY(:,:), DCXDX(:,:), DCYDX(:,:), DCXDY(:,:), DCYDY(:,:)
  REAL,    POINTER :: DCDX(:,:,:), DCDY(:,:,:)
#ifdef W3_SMC
  REAL,    POINTER :: DHDX(:), DHDY(:), DHLMT(:,:)
#endif
...
9000 FORMAT (' TEST W3NAUX : SETTING UP FOR ',I4,' GRIDS') ! W3_T
  ...
  SUBROUTINE W3DIMA  ( IMOD, NDSE, NDST, D_ONLY )
    ...
    USE W3ODATMD   , ONLY : NOSWLL, NOEXTR, UNDEF, FLOGRD, FLOGR2
    ...
    ! 1.  Test input and module status
    !
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 0')
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 1')
    !
    ! -------------------------------------------------------------------- /
    ! 2.  Allocate arrays
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 2')
    !
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 3')
    !
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 4')
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 5')
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 6')
    !
    ! 7) Wave-bottom layer
    !
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 7')
    !
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 9')
    !
    ! 10) User defined
    !
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 10')
    !
    IF ( FL_ALL ) THEN
       !
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 11')
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA 12')
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3DIMA END')
    !
    RETURN
    !
    ! Formats
...
9003 FORMAT (' TEST W3DIMA : DIMENSIONS STORED')
  ...
  SUBROUTINE W3XDMA  ( IMOD, NDSE, NDST, OUTFLAGS )
    ...
    call print_memcheck(30000+IAPROC, 'memcheck_____:'//' W3XDMA')
    !
    RETURN
    !
    ! Formats
...
9003 FORMAT (' TEST W3XDMA : DIMENSIONS STORED')
  ...
  SUBROUTINE W3DMNL  ( IMOD, NDSE, NDST, NSP, NSPX )
    ...
    USE W3GDATMD, ONLY: NSPEC, NTH, GTYPE, UNGTYPE
...
#endif
    !
9000 FORMAT (' TEST W3DMNL : MODEL ',I4) ! W3_T
...
1002 FORMAT (/' *** ERROR W3SETA : ILLEGAL MODEL NUMBER *** '/       &
         ...
         '                    NADATA = ',I10/)
    !
9000 FORMAT (' TEST W3SETA : MODEL ',I4,' SELECTED') ! W3_T
    !/
    !/ End of W3SETA ----------------------------------------------------- /
    !/
...
1002 FORMAT (/' *** ERROR W3XETA : ILLEGAL MODEL NUMBER *** '/       &
         ...
         '                    NADATA = ',I10/)
    !
9000 FORMAT (' TEST W3XETA : MODEL ',I4,' SELECTED') ! W3_T
    !/
    !/ End of W3XETA ----------------------------------------------------- /
    !/
