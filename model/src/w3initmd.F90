...
CONTAINS
  ...
  SUBROUTINE W3INIT ( IMOD, IsMulti, FEXT, MDS, MTRACE, ODAT , FLGRD, &
    ...
    INTEGER              :: ISTEP, ISP, IW           ! W3_DIST
    INTEGER              :: IERR_MPI, BGROUP, LGROUP ! W3_MPI
    INTEGER, SAVE        :: IENT = 0 ! W3_S
    INTEGER              :: NX0, NXN ! W3_T
    INTEGER, ALLOCATABLE :: MAPOUT(:,:)              ! W3_T
    INTEGER, ALLOCATABLE :: TMPRNK(:)                ! W3_MPI
    INTEGER, ALLOCATABLE :: NT(:), MAPTST(:,:)
    INTEGER, SAVE        :: NXS = 49 ! W3_T
    REAL                 :: DTTST, DEPTH, FRACOS
    REAL                 :: FACTOR
    REAL                 :: WLVeff
    REAL, ALLOCATABLE    :: XOUT(:,:) ! W3_T
    LOGICAL              :: OPENED
    CHARACTER(LEN=8)     :: STTIME
    CHARACTER(LEN=10)    :: STDATE
    INTEGER              :: ISPROC
    CHARACTER(LEN=12)    :: FORMAT                   ! W3_DIST
    CHARACTER(LEN=23)    :: DTME21
    CHARACTER(LEN=30)    :: LFILE, TFILE
    INTEGER              :: IScal(1), IPROC          ! W3_PDLIB
    !/
    !/ ------------------------------------------------------------------- /
    !
    ...
    call print_memcheck(10000+IAPROC, 'memcheck_____:'//' WW3_INIT SECTION 1')

    CALL W3SETO ( IMOD, MDS(2), MDS(3) )

    call print_memcheck(10000+IAPROC, 'memcheck_____:'//' WW3_INIT SECTION 1a')

    CALL W3SETG ( IMOD, MDS(2), MDS(3) )

    call print_memcheck(10000+IAPROC, 'memcheck_____:'//' WW3_INIT SECTION 1b')

    CALL W3SETW ( IMOD, MDS(2), MDS(3) )

    call print_memcheck(10000+IAPROC, 'memcheck_____:'//' WW3_INIT SECTION 1c')
    ...
    call print_memcheck(10000+IAPROC, 'memcheck_____:'//' WW3_INIT SECTION 1e')
    !
    !
...
!!!/PDLIB    CALL W3SETG(IMOD, NDSE, NDST)
    ...
    IF ( MDS(3).NE.MDS(1) .AND. MDS(3).NE.MDS(4) .AND. TSTOUT ) THEN
       ...
       IF ( .NOT. OPENED ) then
          OPEN (MDS(3),FILE=FNMPRE(:J)//TFILE(:IFT),ERR=889,IOSTAT=IERR)
       end IF
    END IF
    !
    ...
    !
    call print_memcheck(10000+IAPROC, 'memcheck_____:'//' WW3_INIT SECTION 2')
    !
    ...
    call print_memcheck(10000+IAPROC, 'memcheck_____:'//' WW3_INIT SECTION 2f')
    !
    if (w3_debuginit_flag) then
    ...
    call print_memcheck(10000+IAPROC, 'memcheck_____:'//' WW3_INIT SECTION 3')
    !
    ...
    call print_memcheck(10000+IAPROC, 'memcheck_____:'//' WW3_INIT SECTION 3a')
    ...
    call print_memcheck(10000+IAPROC, 'memcheck_____:'//' WW3_INIT SECTION 4')
    !
    ...
    call print_memcheck(10000+IAPROC, 'memcheck_____:'//' WW3_INIT SECTION 7 - After allocation of group velocities')
    if (w3_debuginit_flag) then
...
9033 FORMAT ( ' +------+------+------+------+--------------+',         &
  ...
  SUBROUTINE W3MPIO ( IMOD )
    ...
    !/
#ifdef W3_MPI
    INTEGER          :: IK, IFJ                      ! W3_MPI
    INTEGER          :: IH, IT0, IROOT, IT, IERR, I0 ! W3_MPI
    INTEGER          :: IFROM, IX(4), IY(4), IS(4)   ! W3_MPI
    INTEGER          :: IP(4), I, J, JSEA, ITARG, IB ! W3_MPI
    INTEGER          :: JSEA0, JSEAN, NSEAB, IBOFF   ! W3_MPI
...
9043 FORMAT ( ' TEST W3MPIO: NRQTR :',I10)
  ...
  SUBROUTINE W3MPIP ( IMOD )
    ...
    USE W3ODATMD, ONLY: NDST, NDSE, IAPROC, NAPROC, NAPPNT, FLOUT ! W3_MPI
    USE W3ODATMD, ONLY: OUTPTS, NRQPO, NRQPO2, IRQPO1, IRQPO2     ! W3_MPI
