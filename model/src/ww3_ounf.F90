...
!        READ (NDSI,*,END=801, ERR=802) TTYPE, TIMEUNIT
        ...
        READ (NDSI,*,END=801,ERR=802) SMCTYPE
        IF(SMCTYPE .EQ. 1) THEN  ! Flat sea point output
           CALL NEXTLN ( COMSTR , NDSI , NDSE )
           READ (NDSI,*,END=801,ERR=802) SXO, SYO, EXO, EYO
        ELSE IF(SMCTYPE .EQ. 2) THEN  ! Regular grid output
           CALL NEXTLN ( COMSTR , NDSI , NDSE )
...
!
      ...
      ELSE IF(SMCTYPE .EQ. 3 .OR. SMCTYPE .EQ. 4) THEN  ! Nearest neighbour interpolation
        CALL READ_SMCINT()
      ENDIF
...
!
      IF(SMCTYPE .EQ. 2) CALL MAPSTA_SMC()
#endif // W3_SMC
!
!--- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
