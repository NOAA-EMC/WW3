...
CONTAINS
  ...
  !> @date   22-Mar-2021
  !>
  SUBROUTINE W3SRCE ( srce_call, IT, ISEA_OLD, JSEA_OLD, IX_OLD, IY_OLD, IMOD,          &
    ...
    !/ Parameter list
    !/
    INTEGER, INTENT(IN)     :: srce_call, IT, ISEA_OLD, JSEA_OLD, IX_OLD, IY_OLD, IMOD
    ...
    INTEGER :: CHUNK0, CHUNKN, NSEAC, I, ISEA, JSEA, IX, IY
    ! --- END GPU REFACTOR --- !
...
!CB        CG1_CHUNK(:,I) = CG1(1:NK,ISEA)
!CB        WN1_CHUNK(:,I) = WN1(1:NK,ISEA)
!CB        U10_CHUNK(I) = U10ABS(ISEA)
!CB        U10D_CHUNK(I) = U10DIR(ISEA)
!CB        UST_CHUNK(I) = USTAR(ISEA)
!CB        USTD_CHUNK(I) = USTDIR(ISEA)
...
!$ACC END KERNELS
      ...
      !
#ifdef W3_ST0
      CALL W3SPR0 (SPEC, CG1(:,JSEA), WN1(:,JSEA), EMEAN, FMEAN, WNMEAN, AMAX)
      FP     = 0.85 * FMEAN
#endif
#ifdef W3_ST1
      CALL W3SPR1 (SPEC, CG1(:,JSEA), WN1(:,JSEA), EMEAN, FMEAN, WNMEAN, AMAX)
      FP     = 0.85 * FMEAN
#endif
#ifdef W3_ST2
      CALL W3SPR2 (SPEC, CG1(:,JSEA), WN1(:,JSEA), DEPTH, FPI, U10ABS, USTAR,    & 
      ...
      IF ( IT .eq. 0 ) THEN
        ...
        USTAR=0.
        USTDIR=0.
        CALL W3SPR3 (SPEC, CG1(:,JSEA), WN1(:,JSEA), EMEAN, FMEAN, FMEANS, WNMEAN, &
      ...
      ELSE
        CALL W3SPR3 (SPEC, CG1(:,JSEA), WN1(:,JSEA), EMEAN, FMEAN, FMEANS, WNMEAN, &
             AMAX, U10ABS, U10DIR, USTAR, USTDIR,          &
             TAUWX, TAUWY, CD, Z0, CHARN, LLWS, FMEANWS)
        CALL W3SIN3 ( SPEC, CG1(:,JSEA), WN2(:,JSEA), U10ABS, USTAR, DRAT, AS,   &
      ...
      IF ( IT .eq. 0 ) THEN
        ...
        USTDIR=0.
      ELSE
        CALL W3SPR4 (SPEC, CG1(:,JSEA), WN1(:,JSEA), EMEAN, FMEAN, FMEAN1, WNMEAN, &
             AMAX, U10ABS, U10DIR,                           &
#ifdef W3_FLX5
             TAUA, TAUADIR, DAIR,                             &
#endif
...
#endif
#ifdef W3_ST6
      CALL W3SPR6 (SPEC, CG1(:,JSEA), WN1(:,JSEA), EMEAN, FMEAN, WNMEAN, AMAX, FP)
#endif
      !
      ...
      DO
        ...
        !
#ifdef W3_LN1
        CALL W3SLN1 (       WN1(:,JSEA), FHIGH, USTAR, U10DIR , VSLN       )
#endif
        !
#ifdef W3_ST1
        CALL W3SIN1 ( SPEC, WN2(:,JSEA), USTAR, U10DIR ,        VSIN, VDIN )
#endif
#ifdef W3_ST2
        CALL W3SIN2 ( SPEC, CG1(:,JSEA), WN2(:,JSEA), U10ABS, U10DIR, CD, Z0,        &
             FPI, VSIN, VDIN )
#endif
#ifdef W3_ST3
        CALL W3SIN3 ( SPEC, CG1(:,JSEA), WN2(:,JSEA), U10ABS, USTAR, DRAT, AS,       &
...
#ifdef W3_ST6
        CALL W3SIN6 ( SPEC, CG1(:,JSEA), WN2(:,JSEA), U10ABS, USTAR, USTDIR, CD, DAIR, &
        ...
        !
#ifdef W3_ST1
        CALL W3SDS1 ( SPEC, WN2(:,JSEA), EMEAN, FMEAN, WNMEAN,  VSDS, VDDS )
#endif
#ifdef W3_ST2
        CALL W3SDS2 ( SPEC, CG1(:,JSEA), WN1(:,JSEA), FPI, USTAR, ALPHA,VSDS, VDDS )
#endif
#ifdef W3_ST3
        CALL W3SDS3 ( SPEC, WN1(:,JSEA), CG1(:,JSEA), EMEAN, FMEANS, WNMEAN,              &
...
#endif
#ifdef W3_DB1
          CALL W3SDB1 ( IX, SPEC, DEPTH, EMEAN, FMEAN, WNMEAN, CG1(:,JSEA),       &
               LBREAK, VSDB, VDDB )
#endif
#ifdef W3_PDLIB
        ENDIF
...
8888  FORMAT (1X,I8.8,1X,I6.6,F8.1,L2,F8.2)
        ...
        AFILT  = MAX ( DAM(NSPEC,JSEA) , XFLT*AMAX )   !! TODO
        !
...
!W3_PDLIB
        ...
        !
#ifdef W3_NLS
        CALL W3SNLS ( SPEC, CG1(:,JSEA), WN1(:,JSEA), DEPTH, U10ABS, DT, AA=SPEC )
#endif
        !
        ! 6.  Add tail ------------------------------------------------------- *
        !   a Mean parameters
...
#endif
#ifdef W3_ST3
        CALL W3SPR3 (SPEC, CG1(:,JSEA), WN1(:,JSEA), EMEAN, FMEAN, FMEANS,        &
...
888 CONTINUE
      ...
      IF (REFLEC(1).GT.0.OR.REFLEC(2).GT.0.OR.(REFLEC(4).GT.0.AND.BERG.GT.0)) THEN
        CALL W3SREF ( SPEC, CG1(:,JSEA), WN1(:,JSEA), EMEAN, FMEAN, DEPTH, CX, CY,   &
