...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine scrip(src_num, dst_num, l_master, l_read, l_test)
    ...
    use scrip_remap_read                 ! routines for remap input
#endif
...
#ifdef W3_SCRIPNC
    character (LEN=3) :: cdst  ! 3 character number of destination map
    character (LEN=3) :: csrc  ! 3 character number of source map
#endif
#ifdef W3_T38
    CHARACTER (LEN=10) :: CDATE_TIME(3)
    INTEGER            :: DATE_TIME(8)
    INTEGER            :: ELAPSED_TIME, BEG_TIME, END_TIME
#endif
...
#ifdef W3_T38
    if(l_master)write(SCRIP_stdout,*)'subroutine scrip'
#endif
    ...
    num_maps = 1
#ifdef W3_SCRIPNC
    !     Note: Only master does I/O, but all processors need to know about
    !     file existence
    interp_file1 = "rmp_src_to_dst_conserv_XXX_XXX.nc"
    interp_file2 = 'not_used.nc'
    map1_name = 'source to destination Conservative Mapping'
    map2_name = 'map not used'
    write(cdst, "(i3.3)") dst_num
    write(csrc, "(i3.3)") src_num
...
#ifdef W3_T38
    if(l_master)write(SCRIP_stdout,*)'calling grid_init'
#endif

    call grid_init( errorCode,l_master,l_test)

#ifdef W3_T38
    if(l_master)write(SCRIP_stdout, *) 'Computing remappings between: ',grid1_name
    if(l_master)write(SCRIP_stdout, *) '                         and  ',grid2_name
#endif
