...
#ifdef W3_PDLIB
      USE yowNodepool, only: ListIPLG, ListNP, ListNPA
...
!
      ...
      IF (GRIDS(IMOD)%GTYPE .ne. UNGTYPE) THEN
        ...
        WRITE(740+IAPROC,*) 'Exiting because the grid is not unstructured'
        FLUSH(740+IAPROC)
#endif
        RETURN
      END IF
#ifdef W3_DEBUGNETCDF
        WRITE(740+IAPROC,*) 'Entering the NETCDF output routine'
        FLUSH(740+IAPROC)
#endif
      !
      ...
      DO IX=1,NX
        ...
        eIOBP=IOBP(IX)
#ifdef W3_DEBUGNETCDF
        WRITE(740+IAPROC,*) 'IX=', IX, ' IOBP/MAP=', eIOBP, eMap
#endif
        IF (ISEA .eq. 0) THEN
      ...
      END DO
#ifdef W3_DEBUGNETCDF
        WRITE(740+IAPROC,*) 'nbIOBP0=', nbIOBP0
        WRITE(740+IAPROC,*) 'nbIOBP1=', nbIOBP1
        WRITE(740+IAPROC,*) 'nbMAP2_IOBP0=', nbMAP2_IOBP0
        WRITE(740+IAPROC,*) 'nbBound_ISEA=', nbBound_ISEA
        WRITE(740+IAPROC,*) 'nbBound_IOBP=', nbBound_IOBP
        WRITE(740+IAPROC,*) 'sumIOBP_NX=', sumIOBP_NX
        WRITE(740+IAPROC,*) 'sumIOBP_relevant=', sumIOBP_relevant
        FLUSH(740+IAPROC)
#endif
...
!          WRITE(*,*) 'ET/ETX/ETY=', ET, ETX, ETY
...
#ifdef W3_SETUP
        VAR_TOT_NSEA(14, ISEA)=ZETA_SETUP(ISEA)
#endif
        Status(ISEA)=1
      END DO
      !
      ...
      IF (IAPROC .eq. 1) THEN
        DO iProc=2,NAPROC
          CALL MPI_RECV(rVect,NSEA*nVar,MPI_REAL, iProc-1, 19, MPI_COMM_WCMP, istatus, ierr)
          CALL MPI_RECV(rStatus,NSEA,MPI_INTEGER, iProc-1, 23, MPI_COMM_WCMP, istatus, ierr)
          DO I=1,NSEA
            IF (rStatus(I) .eq. 1) THEN
              VAR_TOT_NSEA(:,I)=rVect(:,I)
              Status(I)=1
            END IF
          END DO
        END DO
      ELSE
        CALL MPI_SEND(VAR_TOT_NSEA,NSEA*nVar,MPI_REAL, 0, 19, MPI_COMM_WCMP, ierr)
        CALL MPI_SEND(Status,NSEA,MPI_INTEGER, 0, 23, MPI_COMM_WCMP, ierr)
      END IF
#endif
      ...
      IF (GTYPE .eq. UNGTYPE) THEN
        ...
        ELSE
          iVect(1)=nbexport
          CALL MPI_SEND(iVect,1,MPI_INT, 0, 19, MPI_COMM_WCMP, ierr)
          IF (nbexport .gt. 0) THEN
            allocate(WBACexch(NK,NTH,nbexport), stat=istat)
            idx=0
            DO IP=1,NP
              IP_glob=iplg(IP)
              IF (IOBP_relevant(IP_glob) .eq. 2) THEN
                ...
                DO IK=1,NK
                  DO ITH=1,NTH
                    ISP    = ITH + (IK-1)*NTH
                    WBACexch(IK,ITH,idx) = VA(ISP,JSEA)
                  END DO
                END DO
              END IF
            END DO
      ...
      IF (IAPROC .eq. 1) THEN
...
#ifdef W3_SETUP
          CALL NETCDF_QAD_DEFINE_VAR("ZETA_SETUP", nx_dims, ny_dims, ntime_dims, ncid)
#endif
          !
        ...
        CALL WW3_TO_JD(TIME0_NETCDF_QAD, eJD)
#ifdef W3_DEBUGNETCDF
        WRITE(740+IAPROC,*) 'OUTPUT_NETCDF_QUICK_AND_DIRTY'
        WRITE(740+IAPROC,*) 'QAD: eJD=', eJD
        WRITE(740+IAPROC,*) 'QAD: DTG=', DTG
        WRITE(740+IAPROC,*) 'QAD: recs_his=', recs_his
#endif
        !
        pos=recs_his+1
#ifdef W3_DEBUGNETCDF
        WRITE(740+IAPROC,*) 'pos=', pos
        FLUSH(740+IAPROC)
#endif
        eVar(1)=eTime
...
#ifdef W3_SETUP
        CALL NETCDF_QAD_PUT_VAR("ZETA_SETUP", 14, nVar, VAR_TOT_NSEA, ncid, pos)
#endif
        IF (GTYPE .eq. UNGTYPE) THEN
          DO IWB=1,IWBMNPGL
            IX=ListIdx(IWB)
          ...
          !
#ifdef W3_DEBUGNETCDF
        WRITE(740+IAPROC,*) 'Before writing of SPPARM'
        WRITE(740+IAPROC,*) 'pos=', pos
        WRITE(740+IAPROC,*) 'IWBMNPGL=', IWBMNPGL
        FLUSH(740+IAPROC)
#endif
          IF (IWBMNPGL .gt. 0) THEN
...
#ifdef W3_DEBUGNETCDF
        WRITE(740+IAPROC,*) 'After writing of SPPARM'
        FLUSH(740+IAPROC)
#endif
            !
        ...
        !
#ifdef W3_DEBUGNETCDF
        WRITE(740+IAPROC,*) 'HS values'
        WRITE(740+IAPROC,*) 'HS(min/max)=', minval(VAR_TOT_NSEA(1,:)), maxval(VAR_TOT_NSEA(1,:))
        WRITE(740+IAPROC,*) 'Ucurr(min/max)=', minval(VAR_TOT_NSEA(2,:)), maxval(VAR_TOT_NSEA(2,:))
        WRITE(740+IAPROC,*) 'Vcurr(min/max)=', minval(VAR_TOT_NSEA(3,:)), maxval(VAR_TOT_NSEA(3,:))
        WRITE(740+IAPROC,*) 'sum(varWrite)=', sum(varWrite)
        FLUSH(740+IAPROC)
#endif
        !
        iret=nf90_close(ncid)
...
#ifdef W3_DEBUGNETCDF
        WRITE(740+IAPROC,*) 'Leaving the NETCDF output routine'
        FLUSH(740+IAPROC)
#endif
      END SUBROUTINE
!**********************************************************************
