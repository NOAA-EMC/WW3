...
CONTAINS
  ...
  SUBROUTINE W3WAVE ( IMOD, ODAT, TEND, STAMP, NO_OUT, ID_LCOMM, TIMEN)
    ...
    SUBROUTINE W3WAVE ( IMOD, ODAT, TEND, STAMP, NO_OUT)
      ...
      USE W3ODATMD    , only : IAPROC, IOUTP, NOTYPE, NAPBPT, NTPROC
      ...
      USE W3GDATMD    , only : FSSOURCE, FSTOTALEXP, FSTOTALIMP
      USE W3GDATMD    , only : FSN, FSPSI, FSFCT, FSNIMP, FLCTH
#ifdef W3_PR1
...
#endif
      USE W3PARALL      , only : PRINT_MY_TIME ! W3_TIMINGS
      ...
      REAL                 :: ICEDAVE
      !
      LOGICAL              :: SBSED            ! only for W3_MPI
      INTEGER              :: ISEC1            ! only for W3_SEC1
      INTEGER              :: JJ, NDSOFLG      ! only for W3_SBS
      INTEGER              :: IERR_MPI, NRQMAX ! only for W3_MPI
      INTEGER              :: NAPRST           ! only for W3_MPI
      ...
      REAL                 :: DTGTEMP ! only for W3_SEC1
      !
      REAL, ALLOCATABLE    :: FIELD(:)
      REAL                 :: TMP1(4), TMP2(3), TMP3(2), TMP4(2)
      REAL, ALLOCATABLE    :: WN_I(:) ! only for W3_IC3
      REAL, ALLOCATABLE    :: CIK(:) ! only for W3_REFRX
      !
      ...
      !
      LOGICAL              :: FLGMPI(0:8)   ! only for W3_MPI
      REAL                 :: FIXEDVISC,FIXEDDENS,FIXEDELAS    ! only for W3_IC3
      ...
      INTEGER              :: eIOBP
      INTEGER              :: ITH_F
      REAL                 :: VS_SPEC(NSPEC) ! only for W3_PDLIB
      REAL                 :: VD_SPEC(NSPEC)
      CHARACTER(LEN=30)    :: FOUTNAME ! only for W3_SBS
      REAL                 :: INDSORT(NSEA), DTCFL1(NSEA) ! only for W3_T
      ...
      DO
         ...
         !TODO: Clean up serial W3_IC3 ifdefs
#ifdef W3_IC3
         USE_CHENG=IC3PARS(9)
                  ...
                  IF ( DTTST .EQ. 0. ) THEN
                     if (do_gridded_output) then
                        ...
                        if (user_netcdf_grdout) then
                           !TODO: why is this using FLGMPI(0)? The block netcdf/binary blocks
                           ! could be combined w/o this
#ifdef W3_MPI
...
9010  FORMAT ( ' |',I4,' |',8A15)
    ...
    SUBROUTINE W3SCAT ( ISPEC, MAPSTA, FIELD )
      ...
      !/
      USE W3WDATMD, ONLY: A => VA
      USE W3ADATMD  , ONLY : MPIBUF, BSTAT, IBFLOC, ISPLOC, BISPL, NSPLOC, NRQSG2, IRQSG2, SSTORE ! W3_MPI
