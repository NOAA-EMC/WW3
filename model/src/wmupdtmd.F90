...
!
      IF ( GRIDS(IMOD)%GTYPE .EQ. CLGTYPE .OR. &
        ...
        XGRDI => REAL(GRIDS(JMOD)%XGRD)   !LONS FOR INPUT FIELD
        YGRDI => REAL(GRIDS(JMOD)%YGRD)    !LATS FOR INPUT FIELD

!       GETTING THE INFO FOR THE CURVILINEAR GRID 
        XGRDC => REAL(GRIDS(IMOD)%XGRD)     !LONS FOR CURVI GRID
        YGRDC => REAL(GRIDS(IMOD)%YGRD)     !LATS FOR CURVI GRID
