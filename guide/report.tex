...
version & svn rev.     & date    & comment    \\ \hline \hline
  0.1   &   7869       & May 14, 2010 & Initial MMAB No. 286.      \\ 
        ...
        &              &              & as placeholder only \\
  1.0   & \SVNRevision &    \today    & Adding svn note p.~\pageref{svn_n}  \\
(draft) &              &              & Adding svn warning p.~\pageref{svn_w} \\
        &              &              & Section~\ref{sec:testing} updated  \\
% 1.1   & \SVNRevision &    \today    & Draft work version         \\
\hline \end{tabular}
\end{center}
\vspace{-3mm}
...
The wave model is distributed with a number of test cases, ranging from simple
test to assure that basic features such as propagation and source terms work,
to a realistic tests that include wave hindcast cases in the Atlantic and
Pacific oceans, and a version of the first multi-grid wave model implemented
in operations at NCEP. The following naming conventions have been used in the
test cases:

\begin{itemize}
\item {\file ww3\_tp1.{\it N}}: Test for one-dimensional propagation.
\item {\file ww3\_tp2.{\it N}}: Tests for two-dimensional propagation.
...
\item {\file mww3\_case\_{\it NN}}: Real-world application of the multi-grid
       wave model.
\end{itemize}

\noindent
Particularly the simple idealized test cases should be used for regression
testing when new code is added to the repository; i.e., these test should
result in different answers compared to the previous codes only when
expected. To automate regression testing, \ldots
...
\vspace{\baselineskip}
\begin{center}
\rule[1mm]{35mm}{1.0mm} NOTE TO ERICK AND TIM \rule[1mm]{35mm}{1.0mm} \\ 
\vspace{\baselineskip}
\parbox{120mm}{Please provide a description of the nrl\_test procedure here. I
  would like to move {\file nrl\_test} to plain {\file test} in the
  repository, keeping the present {\file test} and {\file test\_old} if you
  two think there is a need for this. Can you work with Arun and/or
  Henrique to get this done earlier rather than later please?} \\
\vspace{\baselineskip} \rule[1mm]{35mm}{1.0mm} NOTE TO ERICK AND TIM
\rule[1mm]{35mm}{1.0mm}
\end{center}
